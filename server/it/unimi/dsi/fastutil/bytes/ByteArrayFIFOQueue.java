...
public class ByteArrayFIFOQueue implements BytePriorityQueue, Serializable {
   ...
   public ByteArrayFIFOQueue(int var1) {
      super();
      if (var1 < 0) {
         throw new IllegalArgumentException("Initial capacity (" + var1 + ") is negative");
      } else {
         this.array = new byte[Math.max(1, var1)];
   ...
   public byte dequeueByte() {
      if (this.start == this.end) {
         ...
         byte var1 = this.array[this.start];
   ...
   public void enqueueFirst(byte var1) {
      if (this.start == 0) {
   ...
   private void writeObject(ObjectOutputStream var1) throws IOException {
      ...
      while(var2-- != 0) {
         var1.writeByte(this.array[var3++]);
         if (var3 == this.length) {
            var3 = 0;
         }
      }
   ...
   private void readObject(ObjectInputStream var1) throws IOException, ClassNotFoundException {
      ...
      this.array = new byte[this.length = HashCommon.nextPowerOfTwo(this.end + 1)];
