...
public class DoubleArrayFIFOQueue implements DoublePriorityQueue, Serializable {
   ...
   public DoubleArrayFIFOQueue(int var1) {
      super();
      if (var1 < 0) {
         throw new IllegalArgumentException("Initial capacity (" + var1 + ") is negative");
      } else {
         this.array = new double[Math.max(1, var1)];
   ...
   public double dequeueDouble() {
      if (this.start == this.end) {
         ...
         double var1 = this.array[this.start];
   ...
   public double dequeueLastDouble() {
      if (this.start == this.end) {
         ...
         double var1 = this.array[--this.end];
         this.reduce();
         return var1;
      }
   }
   ...
   public void enqueueFirst(double var1) {
      if (this.start == 0) {
   ...
   private void writeObject(ObjectOutputStream var1) throws IOException {
      ...
      while(var2-- != 0) {
         var1.writeDouble(this.array[var3++]);
         if (var3 == this.length) {
            var3 = 0;
         }
      }
