...
public class ShortArrayFIFOQueue implements ShortPriorityQueue, Serializable {
   ...
   public ShortArrayFIFOQueue(int var1) {
      super();
      if (var1 < 0) {
         throw new IllegalArgumentException("Initial capacity (" + var1 + ") is negative");
      } else {
         this.array = new short[Math.max(1, var1)];
   ...
   public short dequeueShort() {
      if (this.start == this.end) {
         ...
         short var1 = this.array[this.start];
   ...
   public short dequeueLastShort() {
      if (this.start == this.end) {
         ...
         short var1 = this.array[--this.end];
         this.reduce();
         return var1;
      }
   }
   ...
   public void enqueueFirst(short var1) {
      if (this.start == 0) {
   ...
   private void writeObject(ObjectOutputStream var1) throws IOException {
      ...
      while(var2-- != 0) {
         var1.writeShort(this.array[var3++]);
         if (var3 == this.length) {
            var3 = 0;
         }
      }
   ...
   private void readObject(ObjectInputStream var1) throws IOException, ClassNotFoundException {
      ...
      this.array = new short[this.length = HashCommon.nextPowerOfTwo(this.end + 1)];
