package it.unimi.dsi.fastutil.booleans;

import it.unimi.dsi.fastutil.BigList;
import java.io.Serializable;
import java.util.Collection;
import java.util.Random;

public final class BooleanBigLists {
   public static final BooleanBigLists.EmptyBigList EMPTY_BIG_LIST = new BooleanBigLists.EmptyBigList();

   private BooleanBigLists() {
      super();
   }

   public static BooleanBigList shuffle(BooleanBigList var0, Random var1) {
      long var2 = var0.size64();

      while(var2-- != 0L) {
         long var4 = (var1.nextLong() & 9223372036854775807L) % (var2 + 1L);
         boolean var6 = var0.getBoolean(var2);
         var0.set(var2, var0.getBoolean(var4));
         var0.set(var4, var6);
      }

      return var0;
   }

   public static BooleanBigList singleton(boolean var0) {
      return new BooleanBigLists.Singleton(var0);
   }

   public static BooleanBigList singleton(Object var0) {
      return new BooleanBigLists.Singleton((Boolean)var0);
   }

   public static BooleanBigList synchronize(BooleanBigList var0) {
      return new BooleanBigLists.SynchronizedBigList(var0);
   }

   public static BooleanBigList synchronize(BooleanBigList var0, Object var1) {
      return new BooleanBigLists.SynchronizedBigList(var0, var1);
   }

   public static BooleanBigList unmodifiable(BooleanBigList var0) {
      return new BooleanBigLists.UnmodifiableBigList(var0);
   }

   public static BooleanBigList asBigList(BooleanList var0) {
      return new BooleanBigLists.ListBigList(var0);
   }

   public static class ListBigList extends AbstractBooleanBigList implements Serializable {
      private static final long serialVersionUID = -7046029254386353129L;
      private final BooleanList list;

      protected ListBigList(BooleanList var1) {
         super();
         this.list = var1;
      }

      private int intIndex(long var1) {
         if (var1 >= 2147483647L) {
            throw new IndexOutOfBoundsException("This big list is restricted to 32-bit indices");
         } else {
            return (int)var1;
         }
      }

      public long size64() {
         return (long)this.list.size();
      }

      public void size(long var1) {
         this.list.size(this.intIndex(var1));
      }

      public BooleanBigListIterator iterator() {
         return BooleanBigListIterators.asBigListIterator(this.list.iterator());
      }

      public BooleanBigListIterator listIterator() {
         return BooleanBigListIterators.asBigListIterator(this.list.listIterator());
      }

      public BooleanBigListIterator listIterator(long var1) {
         return BooleanBigListIterators.asBigListIterator(this.list.listIterator(this.intIndex(var1)));
      }

      public boolean addAll(long var1, Collection<? extends Boolean> var3) {
         return this.list.addAll(this.intIndex(var1), (Collection)var3);
      }

      public BooleanBigList subList(long var1, long var3) {
         return new BooleanBigLists.ListBigList(this.list.subList(this.intIndex(var1), this.intIndex(var3)));
      }

      public boolean contains(boolean var1) {
         return this.list.contains(var1);
      }

      public boolean[] toBooleanArray() {
         return this.list.toBooleanArray();
      }

      public void removeElements(long var1, long var3) {
         this.list.removeElements(this.intIndex(var1), this.intIndex(var3));
      }

      /** @deprecated */
      @Deprecated
      public boolean[] toBooleanArray(boolean[] var1) {
         return this.list.toArray(var1);
      }

      public boolean addAll(long var1, BooleanCollection var3) {
         return this.list.addAll(this.intIndex(var1), var3);
      }

      public boolean addAll(BooleanCollection var1) {
         return this.list.addAll(var1);
      }

      public boolean addAll(long var1, BooleanBigList var3) {
         return this.list.addAll(this.intIndex(var1), (BooleanCollection)var3);
      }

      public boolean addAll(BooleanBigList var1) {
         return this.list.addAll(var1);
      }

      public boolean containsAll(BooleanCollection var1) {
         return this.list.containsAll(var1);
      }

      public boolean removeAll(BooleanCollection var1) {
         return this.list.removeAll(var1);
      }

      public boolean retainAll(BooleanCollection var1) {
         return this.list.retainAll(var1);
      }

      public void add(long var1, boolean var3) {
         this.list.add(this.intIndex(var1), var3);
      }

      public boolean add(boolean var1) {
         return this.list.add(var1);
      }

      public boolean getBoolean(long var1) {
         return this.list.getBoolean(this.intIndex(var1));
      }

      public long indexOf(boolean var1) {
         return (long)this.list.indexOf(var1);
      }

      public long lastIndexOf(boolean var1) {
         return (long)this.list.lastIndexOf(var1);
      }

      public boolean removeBoolean(long var1) {
         return this.list.removeBoolean(this.intIndex(var1));
      }

      public boolean set(long var1, boolean var3) {
         return this.list.set(this.intIndex(var1), var3);
      }

      public boolean isEmpty() {
         return this.list.isEmpty();
      }

      public <T> T[] toArray(T[] var1) {
         return this.list.toArray(var1);
      }

      public boolean containsAll(Collection<?> var1) {
         return this.list.containsAll(var1);
      }

      public boolean addAll(Collection<? extends Boolean> var1) {
         return this.list.addAll(var1);
      }

      public boolean removeAll(Collection<?> var1) {
         return this.list.removeAll(var1);
      }

      public boolean retainAll(Collection<?> var1) {
         return this.list.retainAll(var1);
      }

      public void clear() {
         this.list.clear();
      }

      public int hashCode() {
         return this.list.hashCode();
      }
   }

   public static class UnmodifiableBigList extends BooleanCollections.UnmodifiableCollection implements BooleanBigList, Serializable {
      private static final long serialVersionUID = -7046029254386353129L;
      protected final BooleanBigList list;

      protected UnmodifiableBigList(BooleanBigList var1) {
         super(var1);
         this.list = var1;
      }

      public boolean getBoolean(long var1) {
         return this.list.getBoolean(var1);
      }

      public boolean set(long var1, boolean var3) {
         throw new UnsupportedOperationException();
      }

      public void add(long var1, boolean var3) {
         throw new UnsupportedOperationException();
      }

      public boolean removeBoolean(long var1) {
         throw new UnsupportedOperationException();
      }

      public long indexOf(boolean var1) {
         return this.list.indexOf(var1);
      }

      public long lastIndexOf(boolean var1) {
         return this.list.lastIndexOf(var1);
      }

      public boolean addAll(long var1, Collection<? extends Boolean> var3) {
         throw new UnsupportedOperationException();
      }

      public void getElements(long var1, boolean[][] var3, long var4, long var6) {
         this.list.getElements(var1, var3, var4, var6);
      }

      public void removeElements(long var1, long var3) {
         throw new UnsupportedOperationException();
      }

      public void addElements(long var1, boolean[][] var3, long var4, long var6) {
         throw new UnsupportedOperationException();
      }

      public void addElements(long var1, boolean[][] var3) {
         throw new UnsupportedOperationException();
      }

      /** @deprecated */
      @Deprecated
      public void size(long var1) {
         this.list.size(var1);
      }

      public long size64() {
         return this.list.size64();
      }

      public BooleanBigListIterator iterator() {
         return this.listIterator();
      }

      public BooleanBigListIterator listIterator() {
         return BooleanBigListIterators.unmodifiable(this.list.listIterator());
      }

      public BooleanBigListIterator listIterator(long var1) {
         return BooleanBigListIterators.unmodifiable(this.list.listIterator(var1));
      }

      public BooleanBigList subList(long var1, long var3) {
         return BooleanBigLists.unmodifiable(this.list.subList(var1, var3));
      }

      public boolean equals(Object var1) {
         return var1 == this ? true : this.list.equals(var1);
      }

      public int hashCode() {
         return this.list.hashCode();
      }

      public int compareTo(BigList<? extends Boolean> var1) {
         return this.list.compareTo(var1);
      }

      public boolean addAll(long var1, BooleanCollection var3) {
         throw new UnsupportedOperationException();
      }

      public boolean addAll(BooleanBigList var1) {
         throw new UnsupportedOperationException();
      }

      public boolean addAll(long var1, BooleanBigList var3) {
         throw new UnsupportedOperationException();
      }

      /** @deprecated */
      @Deprecated
      public Boolean get(long var1) {
         return this.list.get(var1);
      }

      /** @deprecated */
      @Deprecated
      public void add(long var1, Boolean var3) {
         throw new UnsupportedOperationException();
      }

      /** @deprecated */
      @Deprecated
      public Boolean set(long var1, Boolean var3) {
         throw new UnsupportedOperationException();
      }

      /** @deprecated */
      @Deprecated
      public Boolean remove(long var1) {
         throw new UnsupportedOperationException();
      }

      /** @deprecated */
      @Deprecated
      public long indexOf(Object var1) {
         return this.list.indexOf(var1);
      }

      /** @deprecated */
      @Deprecated
      public long lastIndexOf(Object var1) {
         return this.list.lastIndexOf(var1);
      }
   }

   public static class SynchronizedBigList extends BooleanCollections.SynchronizedCollection implements BooleanBigList, Serializable {
      private static final long serialVersionUID = -7046029254386353129L;
      protected final BooleanBigList list;

      protected SynchronizedBigList(BooleanBigList var1, Object var2) {
         super(var1, var2);
         this.list = var1;
      }

      protected SynchronizedBigList(BooleanBigList var1) {
         super(var1);
         this.list = var1;
      }

      public boolean getBoolean(long var1) {
         synchronized(this.sync) {
            return this.list.getBoolean(var1);
         }
      }

      public boolean set(long var1, boolean var3) {
         synchronized(this.sync) {
            return this.list.set(var1, var3);
         }
      }

      public void add(long var1, boolean var3) {
         synchronized(this.sync) {
            this.list.add(var1, var3);
         }
      }

      public boolean removeBoolean(long var1) {
         synchronized(this.sync) {
            return this.list.removeBoolean(var1);
         }
      }

      public long indexOf(boolean var1) {
         synchronized(this.sync) {
            return this.list.indexOf(var1);
         }
      }

      public long lastIndexOf(boolean var1) {
         synchronized(this.sync) {
            return this.list.lastIndexOf(var1);
         }
      }

      public boolean addAll(long var1, Collection<? extends Boolean> var3) {
         synchronized(this.sync) {
            return this.list.addAll(var1, (Collection)var3);
         }
      }

      public void getElements(long var1, boolean[][] var3, long var4, long var6) {
         synchronized(this.sync) {
            this.list.getElements(var1, var3, var4, var6);
         }
      }

      public void removeElements(long var1, long var3) {
         synchronized(this.sync) {
            this.list.removeElements(var1, var3);
         }
      }

      public void addElements(long var1, boolean[][] var3, long var4, long var6) {
         synchronized(this.sync) {
            this.list.addElements(var1, var3, var4, var6);
         }
      }

      public void addElements(long var1, boolean[][] var3) {
         synchronized(this.sync) {
            this.list.addElements(var1, var3);
         }
      }

      /** @deprecated */
      @Deprecated
      public void size(long var1) {
         synchronized(this.sync) {
            this.list.size(var1);
         }
      }

      public long size64() {
         synchronized(this.sync) {
            return this.list.size64();
         }
      }

      public BooleanBigListIterator iterator() {
         return this.list.listIterator();
      }

      public BooleanBigListIterator listIterator() {
         return this.list.listIterator();
      }

      public BooleanBigListIterator listIterator(long var1) {
         return this.list.listIterator(var1);
      }

      public BooleanBigList subList(long var1, long var3) {
         synchronized(this.sync) {
            return BooleanBigLists.synchronize(this.list.subList(var1, var3), this.sync);
         }
      }

      public boolean equals(Object var1) {
         if (var1 == this) {
            return true;
         } else {
            synchronized(this.sync) {
               return this.list.equals(var1);
            }
         }
      }

      public int hashCode() {
         synchronized(this.sync) {
            return this.list.hashCode();
         }
      }

      public int compareTo(BigList<? extends Boolean> var1) {
         synchronized(this.sync) {
            return this.list.compareTo(var1);
         }
      }

      public boolean addAll(long var1, BooleanCollection var3) {
         synchronized(this.sync) {
            return this.list.addAll(var1, var3);
         }
      }

      public boolean addAll(long var1, BooleanBigList var3) {
         synchronized(this.sync) {
            return this.list.addAll(var1, var3);
         }
      }

      public boolean addAll(BooleanBigList var1) {
         synchronized(this.sync) {
            return this.list.addAll(var1);
         }
      }

      /** @deprecated */
      @Deprecated
      public void add(long var1, Boolean var3) {
         synchronized(this.sync) {
            this.list.add(var1, var3);
         }
      }

      /** @deprecated */
      @Deprecated
      public Boolean get(long var1) {
         synchronized(this.sync) {
            return this.list.get(var1);
         }
      }

      /** @deprecated */
      @Deprecated
      public Boolean set(long var1, Boolean var3) {
         synchronized(this.sync) {
            return this.list.set(var1, var3);
         }
      }

      /** @deprecated */
      @Deprecated
      public Boolean remove(long var1) {
         synchronized(this.sync) {
            return this.list.remove(var1);
         }
      }

      /** @deprecated */
      @Deprecated
      public long indexOf(Object var1) {
         synchronized(this.sync) {
            return this.list.indexOf(var1);
         }
      }

      /** @deprecated */
      @Deprecated
      public long lastIndexOf(Object var1) {
         synchronized(this.sync) {
            return this.list.lastIndexOf(var1);
         }
      }
   }

   public static class Singleton extends AbstractBooleanBigList implements Serializable, Cloneable {
      private static final long serialVersionUID = -7046029254386353129L;
      private final boolean element;

      protected Singleton(boolean var1) {
         super();
         this.element = var1;
      }

      public boolean getBoolean(long var1) {
         if (var1 == 0L) {
            return this.element;
         } else {
            throw new IndexOutOfBoundsException();
         }
      }

      public boolean rem(boolean var1) {
         throw new UnsupportedOperationException();
      }

      public boolean removeBoolean(long var1) {
         throw new UnsupportedOperationException();
      }

      public boolean contains(boolean var1) {
         return var1 == this.element;
      }

      public boolean[] toBooleanArray() {
         boolean[] var1 = new boolean[]{this.element};
         return var1;
      }

      public BooleanBigListIterator listIterator() {
         return BooleanBigListIterators.singleton(this.element);
      }

      public BooleanBigListIterator listIterator(long var1) {
         if (var1 <= 1L && var1 >= 0L) {
            BooleanBigListIterator var3 = this.listIterator();
            if (var1 == 1L) {
               var3.nextBoolean();
            }

            return var3;
         } else {
            throw new IndexOutOfBoundsException();
         }
      }

      public BooleanBigList subList(long var1, long var3) {
         this.ensureIndex(var1);
         this.ensureIndex(var3);
         if (var1 > var3) {
            throw new IndexOutOfBoundsException("Start index (" + var1 + ") is greater than end index (" + var3 + ")");
         } else {
            return (BooleanBigList)(var1 == 0L && var3 == 1L ? this : BooleanBigLists.EMPTY_BIG_LIST);
         }
      }

      public boolean addAll(long var1, Collection<? extends Boolean> var3) {
         throw new UnsupportedOperationException();
      }

      public boolean addAll(Collection<? extends Boolean> var1) {
         throw new UnsupportedOperationException();
      }

      public boolean removeAll(Collection<?> var1) {
         throw new UnsupportedOperationException();
      }

      public boolean retainAll(Collection<?> var1) {
         throw new UnsupportedOperationException();
      }

      public boolean addAll(BooleanBigList var1) {
         throw new UnsupportedOperationException();
      }

      public boolean addAll(long var1, BooleanBigList var3) {
         throw new UnsupportedOperationException();
      }

      public boolean addAll(long var1, BooleanCollection var3) {
         throw new UnsupportedOperationException();
      }

      public boolean addAll(BooleanCollection var1) {
         throw new UnsupportedOperationException();
      }

      public boolean removeAll(BooleanCollection var1) {
         throw new UnsupportedOperationException();
      }

      public boolean retainAll(BooleanCollection var1) {
         throw new UnsupportedOperationException();
      }

      public void clear() {
         throw new UnsupportedOperationException();
      }

      public long size64() {
         return 1L;
      }

      public Object clone() {
         return this;
      }
   }

   public static class EmptyBigList extends BooleanCollections.EmptyCollection implements BooleanBigList, Serializable, Cloneable {
      private static final long serialVersionUID = -7046029254386353129L;

      protected EmptyBigList() {
         super();
      }

      public boolean getBoolean(long var1) {
         throw new IndexOutOfBoundsException();
      }

      public boolean rem(boolean var1) {
         throw new UnsupportedOperationException();
      }

      public boolean removeBoolean(long var1) {
         throw new UnsupportedOperationException();
      }

      public void add(long var1, boolean var3) {
         throw new UnsupportedOperationException();
      }

      public boolean set(long var1, boolean var3) {
         throw new UnsupportedOperationException();
      }

      public long indexOf(boolean var1) {
         return -1L;
      }

      public long lastIndexOf(boolean var1) {
         return -1L;
      }

      public boolean addAll(long var1, Collection<? extends Boolean> var3) {
         throw new UnsupportedOperationException();
      }

      public boolean addAll(BooleanCollection var1) {
         throw new UnsupportedOperationException();
      }

      public boolean addAll(BooleanBigList var1) {
         throw new UnsupportedOperationException();
      }

      public boolean addAll(long var1, BooleanCollection var3) {
         throw new UnsupportedOperationException();
      }

      public boolean addAll(long var1, BooleanBigList var3) {
         throw new UnsupportedOperationException();
      }

      /** @deprecated */
      @Deprecated
      public void add(long var1, Boolean var3) {
         throw new UnsupportedOperationException();
      }

      /** @deprecated */
      @Deprecated
      public boolean add(Boolean var1) {
         throw new UnsupportedOperationException();
      }

      /** @deprecated */
      @Deprecated
      public Boolean get(long var1) {
         throw new IndexOutOfBoundsException();
      }

      /** @deprecated */
      @Deprecated
      public Boolean set(long var1, Boolean var3) {
         throw new UnsupportedOperationException();
      }

      /** @deprecated */
      @Deprecated
      public Boolean remove(long var1) {
         throw new UnsupportedOperationException();
      }

      /** @deprecated */
      @Deprecated
      public long indexOf(Object var1) {
         return -1L;
      }

      /** @deprecated */
      @Deprecated
      public long lastIndexOf(Object var1) {
         return -1L;
      }

      public BooleanBigListIterator listIterator() {
         return BooleanBigListIterators.EMPTY_BIG_LIST_ITERATOR;
      }

      public BooleanBigListIterator iterator() {
         return BooleanBigListIterators.EMPTY_BIG_LIST_ITERATOR;
      }

      public BooleanBigListIterator listIterator(long var1) {
         if (var1 == 0L) {
            return BooleanBigListIterators.EMPTY_BIG_LIST_ITERATOR;
         } else {
            throw new IndexOutOfBoundsException(String.valueOf(var1));
         }
      }

      public BooleanBigList subList(long var1, long var3) {
         if (var1 == 0L && var3 == 0L) {
            return this;
         } else {
            throw new IndexOutOfBoundsException();
         }
      }

      public void getElements(long var1, boolean[][] var3, long var4, long var6) {
         BooleanBigArrays.ensureOffsetLength(var3, var4, var6);
         if (var1 != 0L) {
            throw new IndexOutOfBoundsException();
         }
      }

      public void removeElements(long var1, long var3) {
         throw new UnsupportedOperationException();
      }

      public void addElements(long var1, boolean[][] var3, long var4, long var6) {
         throw new UnsupportedOperationException();
      }

      public void addElements(long var1, boolean[][] var3) {
         throw new UnsupportedOperationException();
      }

      public void size(long var1) {
         throw new UnsupportedOperationException();
      }

      public long size64() {
         return 0L;
      }

      public int compareTo(BigList<? extends Boolean> var1) {
         if (var1 == this) {
            return 0;
         } else {
            return var1.isEmpty() ? 0 : -1;
         }
      }

      public Object clone() {
         return BooleanBigLists.EMPTY_BIG_LIST;
      }

      public int hashCode() {
         return 1;
      }

      public boolean equals(Object var1) {
         return var1 instanceof BigList && ((BigList)var1).isEmpty();
      }

      public String toString() {
         return "[]";
      }

      private Object readResolve() {
         return BooleanBigLists.EMPTY_BIG_LIST;
      }
   }
}
