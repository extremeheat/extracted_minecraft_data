...
public class FloatArrayFIFOQueue implements FloatPriorityQueue, Serializable {
   ...
   public FloatArrayFIFOQueue(int var1) {
      super();
      if (var1 < 0) {
         throw new IllegalArgumentException("Initial capacity (" + var1 + ") is negative");
      } else {
         this.array = new float[Math.max(1, var1)];
   ...
   public float dequeueFloat() {
      if (this.start == this.end) {
         ...
         float var1 = this.array[this.start];
   ...
   public float dequeueLastFloat() {
      if (this.start == this.end) {
         ...
         float var1 = this.array[--this.end];
         this.reduce();
         return var1;
      }
   }
   ...
   public void enqueueFirst(float var1) {
      if (this.start == 0) {
   ...
   private void writeObject(ObjectOutputStream var1) throws IOException {
      ...
      while(var2-- != 0) {
         var1.writeFloat(this.array[var3++]);
         if (var3 == this.length) {
            var3 = 0;
         }
      }
   ...
   private void readObject(ObjectInputStream var1) throws IOException, ClassNotFoundException {
      ...
      this.array = new float[this.length = HashCommon.nextPowerOfTwo(this.end + 1)];
