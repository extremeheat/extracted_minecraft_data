...
public class TitleScreen extends Screen {
   ...
   protected void init() {
      ...
      }).bounds(this.width / 2 - 100, var4 + 72 + 12, 98, 20).build());
      this.addRenderableWidget(Button.builder(Component.translatable("menu.quit"), (var1x) -> {
         this.minecraft.stop();
      }).bounds(this.width / 2 + 2, var4 + 72 + 12, 98, 20).build());
   ...
   private void createNormalMenuOptions(int var1, int var2) {
      ...
      }).bounds(this.width / 2 - 100, var1 + var2 * 2, 200, 20).tooltip(var5).build())).active = var4;
   }
   ...
   private void createDemoMenuOptions(int var1, int var2) {
      ...
      }).bounds(this.width / 2 - 100, var1 + var2 * 1, 200, 20).build());
