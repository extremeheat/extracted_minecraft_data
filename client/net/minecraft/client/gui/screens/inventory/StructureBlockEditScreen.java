...
public class StructureBlockEditScreen extends Screen {
   ...
   protected void init() {
      ...
      this.saveButton = this.addRenderableWidget(
         new Button(this.width / 2 + 4 + 100, 185, 50, 20, Component.translatable("structure_block.button.save"), var1x -> {
      ...
      this.loadButton = this.addRenderableWidget(
         new Button(this.width / 2 + 4 + 100, 185, 50, 20, Component.translatable("structure_block.button.load"), var1x -> {
      ...
      this.addRenderableWidget(
         CycleButton.<StructureMode>builder(var0 -> Component.translatable("structure_block.mode." + var0.getSerializedName()))
            .withValues(DEFAULT_MODES, ALL_MODES)
            .displayOnlyValue()
            .withInitialValue(this.initialMode)
            .create(this.width / 2 - 4 - 150, 185, 50, 20, Component.literal("MODE"), (var1x, var2x) -> {
               this.structure.setMode(var2x);
               this.updateMode(var2x);
            })
      );
      this.detectButton = this.addRenderableWidget(
         new Button(this.width / 2 + 4 + 100, 120, 50, 20, Component.translatable("structure_block.button.detect_size"), var1x -> {
            if (this.structure.getMode() == StructureMode.SAVE) {
      ...
      this.mirrorButton = this.addRenderableWidget(
         CycleButton.builder(Mirror::symbol)
            ...
            .create(this.width / 2 - 20, 185, 40, 20, Component.literal("MIRROR"), (var1x, var2x) -> this.structure.setMirror(var2x))
      );
      ...
      this.rot0Button = this.addRenderableWidget(new Button(this.width / 2 - 1 - 40 - 1 - 40 - 20, 185, 40, 20, Component.literal("0"), var1x -> {
         this.structure.setRotation(Rotation.NONE);
         this.updateDirectionButtons();
      }));
      this.rot90Button = this.addRenderableWidget(new Button(this.width / 2 - 1 - 40 - 20, 185, 40, 20, Component.literal("90"), var1x -> {
         this.structure.setRotation(Rotation.CLOCKWISE_90);
         this.updateDirectionButtons();
      }));
      this.rot180Button = this.addRenderableWidget(new Button(this.width / 2 + 1 + 20, 185, 40, 20, Component.literal("180"), var1x -> {
         this.structure.setRotation(Rotation.CLOCKWISE_180);
         this.updateDirectionButtons();
      }));
      this.rot270Button = this.addRenderableWidget(new Button(this.width / 2 + 1 + 40 + 1 + 20, 185, 40, 20, Component.literal("270"), var1x -> {
      ...
      this.nameEdit = new EditBox(this.font, this.width / 2 - 152, 40, 300, 20, Component.translatable("structure_block.structure_name")) {
         ...
         public boolean charTyped(char var1, int var2) {
            return !StructureBlockEditScreen.this.isValidCharacterForName(this.getValue(), var1, this.getCursorPosition())
               ? false
               : super.charTyped(var1, var2);
         }
      };
