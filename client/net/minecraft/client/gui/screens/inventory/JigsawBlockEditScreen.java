...
public class JigsawBlockEditScreen extends Screen {
   ...
   protected void init() {
      ...
      this.addRenderableWidget(CycleButton.onOffBuilder(this.keepJigsaws).create(this.width / 2 - 50, 180, 100, 20, Component.translatable("jigsaw_block.keep_jigsaws"), (var1x, var2x) -> {
         this.keepJigsaws = var2x;
      }));
      this.generateButton = (Button)this.addRenderableWidget(new Button(this.width / 2 + 54, 180, 100, 20, Component.translatable("jigsaw_block.generate"), (var1x) -> {
         this.onDone();
         this.sendGenerate();
      }));
