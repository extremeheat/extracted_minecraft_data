...
public class FogRenderer implements AutoCloseable {
   ...
   public static Vector4f computeFogColor(Camera var0, float var1, ClientLevel var2, int var3, float var4) {
      ...
      if (var5 == FogType.WATER) {
         ...
         float var31 = ARGB.redFloat(var11);
         float var34 = ARGB.greenFloat(var11);
         float var37 = ARGB.blueFloat(var11);
         ...
         Vec3 var41 = CubicSampler.gaussianSampleVec3(var40, (var3x, var4x, var5x) -> var2.effects().getBrightnessDependentFogColor(Vec3.fromRGB24(((Biome)var39.getNoiseBiomeAtQuart(var3x, var4x, var5x).value()).getFogColor()), var38));
         var7 = (float)var41.x();
         var8 = (float)var41.y();
         var9 = (float)var41.z();
         if (var3 >= 4) {
            ...
            if (var47 > 0.0F && var2.effects().isSunriseOrSunset(var2.getTimeOfDay(var1))) {
               ...
               var47 *= ARGB.alphaFloat(var51);
               var7 = var7 * (1.0F - var47) + ARGB.redFloat(var51) * var47;
               var8 = var8 * (1.0F - var47) + ARGB.greenFloat(var51) * var47;
               var9 = var9 * (1.0F - var47) + ARGB.blueFloat(var51) * var47;
            }
         }
      ...
      float var28 = ((float)var0.getPosition().y - (float)var2.getMinY()) * var2.getLevelData().getClearColorScale();
      MobEffectFogFunction var30 = getPriorityFogFunction(var6, var1);
      if (var30 != null) {
      ...
      if (var5 == FogType.WATER) {
         ...
         label86: {
            if (var6 instanceof LivingEntity) {
               LivingEntity var35 = (LivingEntity)var6;
               if (var35.hasEffect(MobEffects.NIGHT_VISION) && !var35.hasEffect(MobEffects.DARKNESS)) {
   ...
   private static MobEffectFogFunction getPriorityFogFunction(Entity var0, float var1) {
      if (var0 instanceof LivingEntity var2) {
         return (MobEffectFogFunction)MOB_EFFECT_FOG.stream().filter((var2x) -> var2x.isEnabled(var2, var1)).findFirst().orElse((Object)null);
      } else {
         return null;
      }
   }

   public void setupFog(Camera var1, Vector4f var2, float var3, boolean var4, float var5) {
      FogType var6 = var1.getFluidInCamera();
      Entity var7 = var1.getEntity();
      FogData var8 = new FogData();
      ...
      if (var6 == FogType.LAVA) {
         ...
         if (var7 instanceof LocalPlayer) {
            LocalPlayer var15 = (LocalPlayer)var7;
            var8.end *= Math.max(0.25F, var15.getWaterVision());
            Holder var18 = var15.level().getBiome(var15.blockPosition());
            if (var18.is(BiomeTags.HAS_CLOSER_WATER_FOG)) {
               var8.end *= 0.85F;
            }
         }
         ...
         float var16 = Mth.clamp(var3 / 10.0F, 4.0F, 64.0F);
         var8.start = var3 - var16;
         var8.end = var3;
         var8.shape = FogShape.CYLINDER;
         var8.skyEnd = var8.end;
      ...
      try (GpuBuffer.MappedView var17 = RenderSystem.getDevice().createCommandEncoder().mapBuffer(this.regularBuffer.currentBuffer(), false, true)) {
         this.updateBuffer(var17.data(), 0, var2, var8.shape, var8.start, var8.end, var8.skyEnd, var8.cloudEnd);
      }

   }

   private void updateBuffer(ByteBuffer var1, int var2, Vector4f var3, FogShape var4, float var5, float var6, float var7, float var8) {
      var1.position(var2);
      Std140Builder.intoBuffer(var1).putVec4(var3).putInt(var4.getIndex()).putFloat(var5).putFloat(var6).putFloat(var7).putFloat(var8);
   }
   ...
   static class BlindnessFogFunction implements MobEffectFogFunction {
      ...
      public void setupFog(FogData var1, LivingEntity var2, MobEffectInstance var3, float var4, float var5) {
         ...
         var1.end = var6;
         var1.skyEnd = var6 * 0.8F;
         var1.cloudEnd = var6 * 0.8F;
      }
   }
