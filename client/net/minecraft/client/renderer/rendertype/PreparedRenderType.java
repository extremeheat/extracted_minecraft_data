...
public record PreparedRenderType(RenderPipeline pipeline, OutputTarget outputTarget, GpuBufferSlice dynamicTransforms, ScissorState scissorState, List<Texture> textures) {
   ...
   public void drawFromBuffer(final GpuBuffer vertexBuffer, final GpuBuffer indexBuffer, final VertexFormat.IndexType indexType, final int baseVertex, final int firstIndex, final int indexCount) {
      ...
      try (RenderPass renderPass = RenderSystem.getDevice().createCommandEncoder().createRenderPass(() -> "Immediate draw with " + String.valueOf(this.pipeline), colorTexture, OptionalInt.empty(), depthTexture, OptionalDouble.empty())) {
