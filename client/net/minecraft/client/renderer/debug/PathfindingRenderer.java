...
public class PathfindingRenderer implements DebugRenderer.SimpleDebugRenderer {
   ...
   private static final boolean SHOW_STUCK_COUNTDOWN = true;
   private static final int TARGET_NODE_COLOR = -65536;
   private static final int PAST_NODE_COLOR = -8355712;
   private static final int NEXT_NODE_COLOR = -16776961;
   ...
   public void emitGizmos(final double camX, final double camY, final double camZ, final DebugValueAccess debugValues, final Frustum frustum, final float partialTicks) {
      debugValues.forEachEntity(DebugSubscriptions.ENTITY_PATHS, (entity, info) -> renderPath(camX, camY, camZ, info.path(), info.maxNodeDistance(), info.stuckCountdown()));
   }

   private static void renderPath(final double camX, final double camY, final double camZ, final Path path, final float maxNodeDistance, final int stuckCountdown) {
      renderPath(path, maxNodeDistance, stuckCountdown, true, true, camX, camY, camZ);
   }

   public static void renderPath(final Path path, final float maxNodeDistance, final int stuckCountdown, final boolean renderOpenAndClosedSets, final boolean renderGroundLabels, final double camX, final double camY, final double camZ) {
      ...
      if (distanceToCamera(pos, camX, camY, camZ) <= 80.0F) {
         ...
         for(int i = 0; i < path.getNodeCount(); ++i) {
            ...
            if (distanceToCamera(n.asBlockPos(), camX, camY, camZ) <= 80.0F) {
               ...
               boolean isNextNode = i == nextNodeIndex;
               int nodeColor = isNextNode ? -65536 : (i < nextNodeIndex ? -8355712 : -16776961);
               Gizmos.cuboid(aabb, GizmoStyle.fill(nodeColor));
            }
         }
      }
      ...
      if (renderGroundLabels) {
         for(int i = 0; i < path.getNodeCount(); ++i) {
            ...
            if (distanceToCamera(n.asBlockPos(), camX, camY, camZ) <= 80.0F) {
               ...
               boolean isNextNode = i == nextNodeIndex;
               if (isNextNode) {
                  Gizmos.billboardText(Integer.toString(stuckCountdown), new Vec3((double)n.x + 0.5, (double)n.y + 0.25, (double)n.z + 0.5), TextGizmo.Style.forColorAndCentered(-256).withScale(0.32F)).setAlwaysOnTop();
               } else {
