package net.minecraft.world.gen.placement;

import java.util.Random;
import net.minecraft.util.math.BlockPos;
import net.minecraft.world.IWorld;
import net.minecraft.world.gen.Heightmap;
import net.minecraft.world.gen.IChunkGenSettings;
import net.minecraft.world.gen.IChunkGenerator;
import net.minecraft.world.gen.feature.Feature;
import net.minecraft.world.gen.feature.IFeatureConfig;

public class TwiceSurfaceWithChance extends BasePlacement<ChanceConfig> {
   public TwiceSurfaceWithChance() {
      super();
   }

   public <C extends IFeatureConfig> boolean func_201491_a_(IWorld var1, IChunkGenerator<? extends IChunkGenSettings> var2, Random var3, BlockPos var4, ChanceConfig var5, Feature<C> var6, C var7) {
      if (var3.nextFloat() < 1.0F / (float)var5.field_202477_a) {
         int var8 = var3.nextInt(16);
         int var9 = var3.nextInt(16);
         int var10 = var1.func_205770_a(Heightmap.Type.MOTION_BLOCKING, var4.func_177982_a(var8, 0, var9)).func_177956_o() * 2;
         if (var10 <= 0) {
            return false;
         }

         int var11 = var3.nextInt(var10);
         var6.func_212245_a(var1, var2, var3, var4.func_177982_a(var8, var11, var9), var7);
      }

      return true;
   }
}
