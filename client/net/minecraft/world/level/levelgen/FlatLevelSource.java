...
public class FlatLevelSource extends ChunkGenerator<FlatLevelGeneratorSettings> {
   ...
   private Biome getBiomeFromSettings() {
      ...
      while(true) {
         ...
         do {
            if (!var4.hasNext()) {
               boolean var14 = (!((FlatLevelGeneratorSettings)this.settings).isVoidGen() || var1 == Biomes.THE_VOID) && var3.containsKey("decoration");
               if (var14) {
                  ArrayList var15 = Lists.newArrayList();
                  var15.add(GenerationStep.Decoration.UNDERGROUND_STRUCTURES);
                  var15.add(GenerationStep.Decoration.SURFACE_STRUCTURES);
                  GenerationStep.Decoration[] var17 = GenerationStep.Decoration.values();
                  int var19 = var17.length;

                  for(var8 = 0; var8 < var19; ++var8) {
                     GenerationStep.Decoration var21 = var17[var8];
                     if (!var15.contains(var21)) {
                        Iterator var22 = var1.getFeaturesForStep(var21).iterator();

                        while(var22.hasNext()) {
                           var11 = (ConfiguredFeature)var22.next();
                           var2.addFeature(var21, var11);
                        }
                     }
                  }
               }

               BlockState[] var16 = ((FlatLevelGeneratorSettings)this.settings).getLayers();

               for(int var18 = 0; var18 < var16.length; ++var18) {
                  BlockState var20 = var16[var18];
                  if (var20 != null && !Heightmap.Types.MOTION_BLOCKING.isOpaque().test(var20)) {
                     ((FlatLevelGeneratorSettings)this.settings).deleteLayer(var18);
                     var2.addFeature(GenerationStep.Decoration.TOP_LAYER_MODIFICATION, Biome.makeComposite(Feature.FILL_LAYER, new LayerConfiguration(var18, var20), FeatureDecorator.NOPE, DecoratorConfiguration.NONE));
                  }
               }
