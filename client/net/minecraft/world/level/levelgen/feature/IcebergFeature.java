...
public class IcebergFeature extends Feature<BlockStateConfiguration> {
   ...
   public boolean place(FeaturePlaceContext<BlockStateConfiguration> var1) {
      ...
      boolean var5 = var4.nextDouble() > 0.7;
      BlockState var6 = ((BlockStateConfiguration)var1.config()).state;
      double var7 = var4.nextDouble() * 2.0 * 3.141592653589793;
   ...
   private void generateCutOut(RandomSource var1, LevelAccessor var2, int var3, int var4, BlockPos var5, boolean var6, int var7, double var8, int var10) {
   ...
   private void generateIcebergBlock(LevelAccessor var1, RandomSource var2, BlockPos var3, int var4, int var5, int var6, int var7, int var8, int var9, boolean var10, int var11, double var12, boolean var14, BlockState var15) {
      ...
      if (var16 < 0.0) {
         ...
         double var19 = var10 ? -0.5 : (double)(-6 - var2.nextInt(3));
         if (var16 > var19 && var2.nextDouble() > 0.9) {
            return;
         }
   ...
   private void setIcebergBlock(BlockPos var1, LevelAccessor var2, RandomSource var3, int var4, int var5, boolean var6, boolean var7, BlockState var8) {
      BlockState var9 = var2.getBlockState(var1);
      if (var9.isAir() || var9.is(Blocks.SNOW_BLOCK) || var9.is(Blocks.ICE) || var9.is(Blocks.WATER)) {
         boolean var10 = !var6 || var3.nextDouble() > 0.05;
         int var11 = var6 ? 3 : 2;
         if (var7 && !var9.is(Blocks.WATER) && (double)var4 <= (double)var3.nextInt(Math.max(1, var5 / var11)) + (double)var5 * 0.6 && var10) {
   ...
   private double signedDistanceCircle(int var1, int var2, BlockPos var3, int var4, RandomSource var5) {
      float var6 = 10.0F * Mth.clamp(var5.nextFloat(), 0.2F, 0.8F) / (float)var4;
      return (double)var6 + Math.pow((double)(var1 - var3.getX()), 2.0) + Math.pow((double)(var2 - var3.getZ()), 2.0) - Math.pow((double)var4, 2.0);
   }

   private double signedDistanceEllipse(int var1, int var2, BlockPos var3, int var4, int var5, double var6) {
      return Math.pow(((double)(var1 - var3.getX()) * Math.cos(var6) - (double)(var2 - var3.getZ()) * Math.sin(var6)) / (double)var4, 2.0) + Math.pow(((double)(var1 - var3.getX()) * Math.sin(var6) + (double)(var2 - var3.getZ()) * Math.cos(var6)) / (double)var5, 2.0) - 1.0;
   }

   private int heightDependentRadiusRound(RandomSource var1, int var2, int var3, int var4) {
      float var5 = 3.5F - var1.nextFloat();
      float var6 = (1.0F - (float)Math.pow((double)var2, 2.0) / ((float)var3 * var5)) * (float)var4;
   ...
   private int heightDependentRadiusEllipse(int var1, int var2, int var3) {
      float var4 = 1.0F;
      float var5 = (1.0F - (float)Math.pow((double)var1, 2.0) / ((float)var2 * 1.0F)) * (float)var3;
      return Mth.ceil(var5 / 2.0F);
   }

   private int heightDependentRadiusSteep(RandomSource var1, int var2, int var3, int var4) {
