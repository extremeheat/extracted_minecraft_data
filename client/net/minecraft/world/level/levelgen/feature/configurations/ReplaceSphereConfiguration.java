...
public class ReplaceSphereConfiguration implements FeatureConfiguration {
   public static final Codec<ReplaceSphereConfiguration> CODEC = RecordCodecBuilder.create(
      var0 -> var0.group(
               BlockState.CODEC.fieldOf("target").forGetter(var0x -> var0x.targetState),
               BlockState.CODEC.fieldOf("state").forGetter(var0x -> var0x.replaceState),
               IntProvider.codec(0, 12).fieldOf("radius").forGetter(var0x -> var0x.radius)
            )
