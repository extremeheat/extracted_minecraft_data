...
public record NoiseRouter(DensityFunction temperature, DensityFunction vegetation, DensityFunction continents, DensityFunction erosion, DensityFunction depth, DensityFunction ridges, DensityFunction chunkSurfaceLevel, DensityFunction finalDensity) {
   ...
   public NoiseRouter rewrite(final DfRewriteRule rule) {
      return new NoiseRouter(rule.rewrite(this.temperature), rule.rewrite(this.vegetation), rule.rewrite(this.continents), rule.rewrite(this.erosion), rule.rewrite(this.depth), rule.rewrite(this.ridges), rule.rewrite(this.chunkSurfaceLevel), rule.rewrite(this.finalDensity));
   }
