...
public record BiomeCondition(HolderSet<Biome> biomes) implements MaterialCondition {
   public static final MapCodec<BiomeCondition> CODEC = RecordCodecBuilder.mapCodec((i) -> i.group(RegistryCodecs.holderSet(Registries.BIOME).fieldOf("biome_is").forGetter(BiomeCondition::biomes)).apply(i, BiomeCondition::new));
   ...
   public ConditionEvaluator compile(final MaterialRuleContext ruleContext) {
      ...
      return new MaterialRuleContext.LazyYCondition(ruleContext) {
         {
            Objects.requireNonNull(BiomeCondition.this);
         }

         protected boolean compute() {
            return BiomeCondition.this.biomes.contains(this.context.getBiome());
         }
      };
   }
