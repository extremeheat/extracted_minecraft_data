...
public record FloorQuotient(Holder<ContextIntProvider> left, Holder<ContextIntProvider> right) implements ContextIntProvider, BinaryProvider<ContextIntProvider> {
   public static final MapCodec<FloorQuotient> MAP_CODEC;

   public FloorQuotient {
      super();
   }
   ...
   public int getIntUnsafe(final LootContext context) throws ArithmeticException {
      return Math.floorDivExact(((ContextIntProvider)this.left().value()).getIntUnsafe(context), ((ContextIntProvider)this.right().value()).getIntUnsafe(context));
   }

   static {
      MAP_CODEC = BinaryProvider.mapCodec(ContextIntProviders.CODEC, FloorQuotient::new);
   }
}
