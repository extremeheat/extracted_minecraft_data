package net.minecraft.world.level.storage.loot.providers.number.ints;

import com.mojang.serialization.MapCodec;
import net.minecraft.core.Holder;
import net.minecraft.world.level.storage.loot.LootContext;
import net.minecraft.world.level.storage.loot.providers.number.UnaryProvider;

public record Absolute(Holder<ContextIntProvider> input) implements ContextIntProvider, UnaryProvider<ContextIntProvider> {
   public static final MapCodec<Absolute> MAP_CODEC;

   public Absolute {
      super();
   }

   public MapCodec<Absolute> codec() {
      return MAP_CODEC;
   }

   public int getIntUnsafe(final LootContext context) {
      return Math.absExact(((ContextIntProvider)this.input().value()).getIntUnsafe(context));
   }

   static {
      MAP_CODEC = UnaryProvider.codec(ContextIntProviders.CODEC, Absolute::new);
   }
}
