...
public record ScoreboardValue(ScoreboardNameProvider target, String score, Holder<ContextIntProvider> fallback) implements ContextIntProvider {
   public static final MapCodec<ScoreboardValue> MAP_CODEC = RecordCodecBuilder.mapCodec((i) -> i.group(ScoreboardNameProviders.CODEC.fieldOf("target").forGetter(ScoreboardValue::target), Codec.STRING.fieldOf("score").forGetter(ScoreboardValue::score), ContextIntProviders.CODEC.optionalFieldOf("fallback", ContextIntProviders.exactly(0)).forGetter(ScoreboardValue::fallback)).apply(i, ScoreboardValue::new));
   ...
   private @Nullable ReadOnlyScoreInfo getScoreInfo(final LootContext context) {
   ...
   public int getIntUnsafe(final LootContext context) {
      ...
      return scoreInfo != null ? scoreInfo.value() : ((ContextIntProvider)this.fallback.value()).getIntUnsafe(context);
   }
}
