...
public class SetRandomDyesFunction extends LootItemConditionalFunction {
   public static final MapCodec<SetRandomDyesFunction> MAP_CODEC = RecordCodecBuilder.mapCodec((i) -> commonFields(i).and(ContextIntProviders.CODEC.fieldOf("number_of_dyes").forGetter((f) -> f.numberOfDyes)).apply(i, SetRandomDyesFunction::new));
   ...
   public ItemStack run(final ItemStack itemStack, final LootContext context) {
      ...
      int rolls = ((ContextIntProvider)this.numberOfDyes.value()).getInt(context);
      if (rolls <= 0) {
         return itemStack;
      } else {
