...
public class PalettedContainer<T> implements PaletteResize<T> {
   ...
   public static <T> Codec<PalettedContainer<T>> codec(IdMap<T> var0, Codec<T> var1, PalettedContainer.Strategy var2, T var3) {
      return RecordCodecBuilder.create((var2x) -> {
         return var2x.group(var1.mapResult(ExtraCodecs.orElsePartial(var3)).listOf().fieldOf("palette").forGetter(PalettedContainer.DiscData::paletteEntries), Codec.LONG_STREAM.optionalFieldOf("data").forGetter(PalettedContainer.DiscData::storage)).apply(var2x, PalettedContainer.DiscData::new);
      }).comapFlatMap((var2x) -> {
         return read(var0, var2, var2x);
      }, (var2x) -> {
         return var2x.write(var0, var2);
      });
   }
   ...
   private static <T> DataResult<PalettedContainer<T>> read(IdMap<T> var0, PalettedContainer.Strategy var1, PalettedContainer.DiscData<T> var2) {
   ...
   private PalettedContainer.DiscData<T> write(IdMap<T> var1, PalettedContainer.Strategy var2) {
      this.acquire();
      ...
      try {
         ...
         var12 = new PalettedContainer.DiscData(var3.getEntries(), var7);
      } finally {
         this.release();
      }
   ...
   public abstract static class Strategy {
      ...
      public static final PalettedContainer.Strategy SECTION_STATES = new PalettedContainer.Strategy(4) {
         public <A> PalettedContainer.Configuration<A> getConfiguration(IdMap<A> var1, int var2) {
            ...
            switch(var2) {
            case 0:
               var10000 = new PalettedContainer.Configuration(SINGLE_VALUE_PALETTE_FACTORY, var2);
               break;
            case 1:
            case 2:
            case 3:
            ...
            case 7:
            case 8:
               var10000 = new PalettedContainer.Configuration(HASHMAP_PALETTE_FACTORY, var2);
               break;
            default:
               var10000 = new PalettedContainer.Configuration(PalettedContainer.Strategy.GLOBAL_PALETTE_FACTORY, Mth.ceillog2(var1.size()));
            }
      ...
      public static final PalettedContainer.Strategy SECTION_BIOMES = new PalettedContainer.Strategy(2) {
         public <A> PalettedContainer.Configuration<A> getConfiguration(IdMap<A> var1, int var2) {
            ...
            case 2:
            case 3:
               var10000 = new PalettedContainer.Configuration(LINEAR_PALETTE_FACTORY, var2);
               break;
            default:
               var10000 = new PalettedContainer.Configuration(PalettedContainer.Strategy.GLOBAL_PALETTE_FACTORY, Mth.ceillog2(var1.size()));
            }
   ...
   private static record DiscData<T>(List<T> a, Optional<LongStream> b) {
      private final List<T> paletteEntries;
      private final Optional<LongStream> storage;

      DiscData(List<T> var1, Optional<LongStream> var2) {
         super();
