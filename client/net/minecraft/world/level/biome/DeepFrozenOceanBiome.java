...
public class DeepFrozenOceanBiome extends Biome {
   ...
   public DeepFrozenOceanBiome() {
      super((new Biome.BiomeBuilder()).surfaceBuilder(SurfaceBuilder.FROZEN_OCEAN, SurfaceBuilder.CONFIG_GRASS).precipitation(Biome.Precipitation.RAIN).biomeCategory(Biome.BiomeCategory.OCEAN).depth(-1.8F).scale(0.1F).temperature(0.5F).downfall(0.5F).waterColor(3750089).waterFogColor(329011).parent((String)null));
   ...
   protected float getTemperatureNoCache(BlockPos var1) {
      ...
      double var3 = FROZEN_TEMPERATURE_NOISE.getValue((double)var1.getX() * 0.05D, (double)var1.getZ() * 0.05D);
      double var5 = BIOME_INFO_NOISE.getValue((double)var1.getX() * 0.2D, (double)var1.getZ() * 0.2D);
      double var7 = var3 + var5;
      if (var7 < 0.3D) {
         double var9 = BIOME_INFO_NOISE.getValue((double)var1.getX() * 0.09D, (double)var1.getZ() * 0.09D);
         if (var9 < 0.8D) {
            var2 = 0.2F;
         }
      }
      ...
      if (var1.getY() > 64) {
         float var11 = (float)(TEMPERATURE_NOISE.getValue((double)((float)var1.getX() / 8.0F), (double)((float)var1.getZ() / 8.0F)) * 4.0D);
