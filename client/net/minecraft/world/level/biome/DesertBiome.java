...
public final class DesertBiome extends Biome {
   public DesertBiome() {
      super((new Biome.BiomeBuilder()).surfaceBuilder(SurfaceBuilder.DEFAULT, SurfaceBuilder.CONFIG_DESERT).precipitation(Biome.Precipitation.NONE).biomeCategory(Biome.BiomeCategory.DESERT).depth(0.125F).scale(0.05F).temperature(2.0F).downfall(0.0F).waterColor(4159204).waterFogColor(329011).parent((String)null));
      this.addStructureStart(Feature.VILLAGE, new VillageConfiguration("village/desert/town_centers", 6));
      this.addStructureStart(Feature.PILLAGER_OUTPOST, new PillagerOutpostConfiguration(0.004D));
      this.addStructureStart(Feature.DESERT_PYRAMID, FeatureConfiguration.NONE);
