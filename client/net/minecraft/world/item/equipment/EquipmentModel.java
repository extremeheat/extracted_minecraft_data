...
public record EquipmentModel(Map<LayerType, List<Layer>> layers) {
   ...
   public static final Codec<EquipmentModel> CODEC;

   public EquipmentModel(Map<LayerType, List<Layer>> var1) {
      super();
      this.layers = var1;
   }
   ...
   static {
      LAYER_LIST_CODEC = ExtraCodecs.nonEmptyList(EquipmentModel.Layer.CODEC.listOf());
      CODEC = RecordCodecBuilder.create((var0) -> {
         return var0.group(ExtraCodecs.nonEmptyMap(Codec.unboundedMap(EquipmentModel.LayerType.CODEC, LAYER_LIST_CODEC)).fieldOf("layers").forGetter(EquipmentModel::layers)).apply(var0, EquipmentModel::new);
      });
   }
   ...
   public static class Builder {
      ...
      public Builder addHumanoidLayers(ResourceLocation var1, boolean var2) {
         this.addLayers(EquipmentModel.LayerType.HUMANOID_LEGGINGS, EquipmentModel.Layer.leatherDyeable(var1, var2));
         this.addMainHumanoidLayer(var1, var2);
         return this;
      }

      public Builder addMainHumanoidLayer(ResourceLocation var1, boolean var2) {
         return this.addLayers(EquipmentModel.LayerType.HUMANOID, EquipmentModel.Layer.leatherDyeable(var1, var2));
      }
      ...
      public EquipmentModel build() {
         return new EquipmentModel((Map)this.layersByType.entrySet().stream().collect(ImmutableMap.toImmutableMap(Map.Entry::getKey, (var0) -> {
   ...
   public static record Layer(ResourceLocation textureId, Optional<Dyeable> dyeable, boolean usePlayerTexture) {
      public static final Codec<Layer> CODEC = RecordCodecBuilder.create((var0) -> {
         return var0.group(ResourceLocation.CODEC.fieldOf("texture").forGetter(Layer::textureId), EquipmentModel.Dyeable.CODEC.optionalFieldOf("dyeable").forGetter(Layer::dyeable), Codec.BOOL.optionalFieldOf("use_player_texture", false).forGetter(Layer::usePlayerTexture)).apply(var0, Layer::new);
      });
