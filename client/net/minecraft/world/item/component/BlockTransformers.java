...
public class BlockTransformers {
   public static final ResourceKey<BlockTransformer> SHOVEL = createKey("shovel");
   public static final ResourceKey<BlockTransformer> AXE = createKey("axe");
   public static final ResourceKey<BlockTransformer> HOE = createKey("hoe");

   public BlockTransformers() {
      super();
   }

   private static ResourceKey<BlockTransformer> createKey(final String name) {
      return ResourceKey.create(Registries.BLOCK_TRANSFORMER, Identifier.withDefaultNamespace(name));
   }

   public static void bootstrap(final BootstrapContext<BlockTransformer> context) {
      context.register(SHOVEL, new BlockTransformer(List.of(BlockTransformer.BlockTransformData.builder(BlockPredicate.allOf(BlockPredicate.matchesTag(BlockTags.TURNS_INTO_DIRT_PATH), BlockPredicate.matchesTag((Directional)Direction.UP, BlockTags.AIR)), Blocks.DIRT_PATH).sound(SoundEvents.SHOVEL_FLATTEN).disallowedFaces(List.of(Direction.DOWN)).build())));
      context.register(AXE, new BlockTransformer(ImmutableList.builder().addAll(List.of(axeStrippables())).addAll(axe(((BiMap)WeatheringCopper.PREVIOUS_BY_BLOCK.get()).entrySet(), SoundEvents.AXE_SCRAPE, BlockTransformer.TransformParticle.SCRAPE)).addAll(axe(((BiMap)HoneycombItem.WAX_OFF_BY_BLOCK.get()).entrySet(), SoundEvents.AXE_WAX_OFF, BlockTransformer.TransformParticle.WAX_OFF)).build()));
      BlockTransformer.BlockTransformData hoeDefault = BlockTransformer.BlockTransformData.builder(RuleBasedStateProvider.builder().ifTrueThenProvide(BlockPredicate.allOf(BlockPredicate.matchesTag(BlockTags.TURNS_INTO_FARMLAND), BlockPredicate.matchesTag((Directional)Direction.UP, BlockTags.AIR)), Blocks.FARMLAND).ifTrueThenProvide(BlockPredicate.allOf(BlockPredicate.matchesBlocks(Blocks.COARSE_DIRT), BlockPredicate.matchesTag((Directional)Direction.UP, BlockTags.AIR)), Blocks.DIRT).build()).sound(SoundEvents.HOE_TILL).disallowedFaces(List.of(Direction.DOWN)).build();
      BlockTransformer.BlockTransformData hoeRootedDirt = BlockTransformer.BlockTransformData.builder(BlockPredicate.matchesBlocks(Blocks.ROOTED_DIRT), Blocks.DIRT).sound(SoundEvents.HOE_TILL).loot(BuiltInLootTables.TILL_ROOTED_DIRT).dropStrategy(BlockTransformer.DropStrategy.CLICKED_FACE).build();
      context.register(HOE, new BlockTransformer(ImmutableList.builder().add(hoeDefault).add(hoeRootedDirt).build()));
   }

   private static BlockTransformer.BlockTransformData axeStrippables() {
