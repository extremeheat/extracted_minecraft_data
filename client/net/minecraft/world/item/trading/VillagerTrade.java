...
public class VillagerTrade implements Validatable {
   ...
   private VillagerTrade(final TradeCost wants, final Optional<TradeCost> additionalWants, final ItemStackTemplate gives, final Holder<ContextIntProvider> maxUses, final Holder<ContextIntProvider> xp, final Holder<ContextFloatProvider> reputationDiscount, final Optional<Holder<LootItemCondition>> merchantPredicate, final Optional<Holder<LootItemFunction>> givenItemModifier, final Optional<HolderSet<Enchantment>> doubleTradePriceEnchantments) {
      super();
      this.wants = wants;
   ...
   public @Nullable MerchantOffer getOffer(final LootContext lootContext) {
      if (this.merchantPredicate.isPresent() && !((LootItemCondition)((Holder)this.merchantPredicate.get()).value()).test(lootContext)) {
         ...
         if (itemCost.count() < 1) {
            ...
            return additionalItemCost.isPresent() && ((ItemCost)additionalItemCost.get()).count() < 1 ? null : new MerchantOffer(itemCost, additionalItemCost, result, Math.max(((ContextIntProvider)this.maxUses.value()).getInt(lootContext), 1), Math.max(((ContextIntProvider)this.xp.value()).getInt(lootContext), 0), Math.max(((ContextFloatProvider)this.reputationDiscount.value()).getFloat(lootContext), 0.0F));
         }
      }
   }
   ...
   public static class Builder {
      ...
      public Builder(final TradeCost wants, final ItemStackTemplate gives, final Holder<ContextIntProvider> maxUses, final Holder<ContextIntProvider> xp, final Holder<ContextFloatProvider> reputationDiscount) {
         super();
         this.wants = wants;
         this.gives = gives;
         this.maxUses = maxUses;
