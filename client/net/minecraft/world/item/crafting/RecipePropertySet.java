...
public class RecipePropertySet {
   ...
   public static final StreamCodec<RegistryFriendlyByteBuf, RecipePropertySet> STREAM_CODEC = ByteBufCodecs.holderRegistry(Registries.ITEM)
      ...
      .map(var0 -> new RecipePropertySet(Set.copyOf(var0)), var0 -> List.copyOf(var0.items));
   ...
   static RecipePropertySet create(Collection<Ingredient> var0) {
      Set var1 = var0.stream().flatMap(var0x -> var0x.items().stream()).collect(Collectors.toUnmodifiableSet());
