...
public record EnchantmentAttributeEffect(String name, Holder<Attribute> attribute, LevelBasedValue amount, AttributeModifier.Operation operation, UUID uuid) implements EnchantmentLocationBasedEffect {
   public static final MapCodec<EnchantmentAttributeEffect> CODEC = RecordCodecBuilder.mapCodec((var0) -> {
      return var0.group(Codec.STRING.fieldOf("name").forGetter(EnchantmentAttributeEffect::name), Attribute.CODEC.fieldOf("attribute").forGetter(EnchantmentAttributeEffect::attribute), LevelBasedValue.CODEC.fieldOf("amount").forGetter(EnchantmentAttributeEffect::amount), AttributeModifier.Operation.CODEC.fieldOf("operation").forGetter(EnchantmentAttributeEffect::operation), UUIDUtil.STRING_CODEC.fieldOf("uuid").forGetter(EnchantmentAttributeEffect::uuid)).apply(var0, EnchantmentAttributeEffect::new);
   });
