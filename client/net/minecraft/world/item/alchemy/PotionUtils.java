   ...
   public static void addPotionTooltip(ItemStack var0, List<Component> var1, float var2) {
      ...
      if (var3.isEmpty()) {
         ...
         for(var5 = var3.iterator(); var5.hasNext(); var1.add(var7.withStyle(var8.getCategory().getTooltipFormatting()))) {
            ...
            var7 = Component.translatable(var6.getDescriptionId());
            var8 = var6.getEffect();
      ...
      if (!var4.isEmpty()) {
         ...
         while(var5.hasNext()) {
            ...
            if (var16.getOperation() != AttributeModifier.Operation.MULTIPLY_BASE && var16.getOperation() != AttributeModifier.Operation.MULTIPLY_TOTAL) {
               ...
               var17 = var16.getAmount() * 100.0;
            }

            if (var15 > 0.0) {
               var1.add(Component.translatable("attribute.modifier.plus." + var16.getOperation().toValue(), ItemStack.ATTRIBUTE_MODIFIER_FORMAT.format(var17), Component.translatable(((Attribute)var14.getFirst()).getDescriptionId())).withStyle(ChatFormatting.BLUE));
            } else if (var15 < 0.0) {
               var17 *= -1.0;
               var1.add(Component.translatable("attribute.modifier.take." + var16.getOperation().toValue(), ItemStack.ATTRIBUTE_MODIFIER_FORMAT.format(var17), Component.translatable(((Attribute)var14.getFirst()).getDescriptionId())).withStyle(ChatFormatting.RED));
            }
         }
      }
