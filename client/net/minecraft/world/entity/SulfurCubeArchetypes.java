...
public class SulfurCubeArchetypes {
   ...
   public static void bootstrap(final BootstrapContext<SulfurCubeArchetype> context) {
      register(context, REGULAR, ItemTags.SULFUR_CUBE_ARCHETYPE_REGULAR, archetype(1.0F, 0.5F, 0.3F, 0.1F), true);
      register(context, BOUNCY, ItemTags.SULFUR_CUBE_ARCHETYPE_BOUNCY, archetype(2.0F, 0.9F, 0.3F, 0.01F), true);
      register(context, SLOW_FLAT, ItemTags.SULFUR_CUBE_ARCHETYPE_SLOW_FLAT, archetype(-0.7F, 0.2F, 0.3F, 0.1F), false);
      register(context, FAST_FLAT, ItemTags.SULFUR_CUBE_ARCHETYPE_FAST_FLAT, archetype(2.0F, 0.2F, 0.1F, 0.01F), false);
      register(context, LIGHT, ItemTags.SULFUR_CUBE_ARCHETYPE_LIGHT, archetype(1.0F, 1.0F, 0.3F, 1.8F), true);
      register(context, FAST_SLIDING, ItemTags.SULFUR_CUBE_ARCHETYPE_FAST_SLIDING, archetype(-0.5F, 0.1F, 0.05F, 0.01F), false);
      register(context, SLOW_SLIDING, ItemTags.SULFUR_CUBE_ARCHETYPE_SLOW_SLIDING, archetype(-0.8F, 0.1F, 0.05F, 0.01F), false);
      register(context, STICKY, ItemTags.SULFUR_CUBE_ARCHETYPE_STICKY, archetype(2.0F, 0.0F, 2.0F, 0.01F), false);
      register(context, HIGH_RESISTANCE, ItemTags.SULFUR_CUBE_ARCHETYPE_HIGH_RESISTANCE, archetype(-0.7F, 0.2F, 1.0F, 0.01F), false);
   }
   ...
   private static List<Function<ResourceKey<SulfurCubeArchetype>, SulfurCubeArchetype.AttributeEntry>> archetype(final float speed, final float bounce, final float friction, final float drag) {
      return List.of(add(Attributes.KNOCKBACK_RESISTANCE, (double)(-speed)), add(Attributes.BOUNCINESS, (double)bounce), multiply(Attributes.FRICTION_MODIFIER, (double)friction), multiply(Attributes.AIR_DRAG_MODIFIER, (double)drag));
   }

   private static void register(final BootstrapContext<SulfurCubeArchetype> context, final ResourceKey<SulfurCubeArchetype> name, final TagKey<Item> blocks, final List<Function<ResourceKey<SulfurCubeArchetype>, SulfurCubeArchetype.AttributeEntry>> modifiers, final boolean floats) {
      context.register(name, new SulfurCubeArchetype(context.lookup(Registries.ITEM).getOrThrow(blocks), modifiers.stream().map((f) -> (SulfurCubeArchetype.AttributeEntry)f.apply(name)).toList(), floats));
   }
}
