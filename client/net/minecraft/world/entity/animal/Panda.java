...
public class Panda extends Animal {
   ...
   private void afterSneeze() {
      Vec3 var1 = this.getDeltaMovement();
      this.level.addParticle(ParticleTypes.SNEEZE, this.getX() - (double)(this.getBbWidth() + 1.0F) * 0.5 * (double)Mth.sin(this.yBodyRot * 0.017453292F), this.getEyeY() - 0.10000000149011612, this.getZ() + (double)(this.getBbWidth() + 1.0F) * 0.5 * (double)Mth.cos(this.yBodyRot * 0.017453292F), var1.x, 0.0, var1.z);
      this.playSound(SoundEvents.PANDA_SNEEZE, 1.0F, 1.0F);
      List var2 = this.level.getEntitiesOfClass(Panda.class, this.getBoundingBox().inflate(10.0));
   ...
   public static enum Gene {
      ...
      private static final Gene[] BY_ID = (Gene[])Arrays.stream(values()).sorted(Comparator.comparingInt(Gene::getId)).toArray((var0) -> {
         return new Gene[var0];
      });
