...
public class Warden extends Monster implements VibrationListener.VibrationListenerConfig {
   ...
   public static AttributeSupplier.Builder createAttributes() {
      return Monster.createMonsterAttributes().add(Attributes.MAX_HEALTH, 500.0).add(Attributes.MOVEMENT_SPEED, 0.30000001192092896).add(Attributes.KNOCKBACK_RESISTANCE, 1.0).add(Attributes.ATTACK_KNOCKBACK, 1.5).add(Attributes.ATTACK_DAMAGE, 30.0);
   }
   ...
   public void tick() {
      ...
      if (var2 instanceof ServerLevel var1) {
         ((VibrationListener)this.dynamicGameEventListener.getListener()).tick(var1);
   ...
   public void addAdditionalSaveData(CompoundTag var1) {
      ...
      var10000 = VibrationListener.codec(this).encodeStart(NbtOps.INSTANCE, (VibrationListener)this.dynamicGameEventListener.getListener());
      var10001 = LOGGER;
      Objects.requireNonNull(var10001);
      var10000.resultOrPartial(var10001::error).ifPresent((var1x) -> {
   ...
   public void readAdditionalSaveData(CompoundTag var1) {
      ...
      if (var1.contains("listener", 10)) {
         var10000 = VibrationListener.codec(this).parse(new Dynamic(NbtOps.INSTANCE, var1.getCompound("listener")));
         var10001 = LOGGER;
         Objects.requireNonNull(var10001);
         var10000.resultOrPartial(var10001::error).ifPresent((var1x) -> {
            this.dynamicGameEventListener.updateListener(var1x, this.level);
         });
      }
   ...
   public boolean hurt(DamageSource var1, float var2) {
      ...
      if (!this.level.isClientSide && !this.isNoAi() && !this.isDiggingOrEmerging()) {
         ...
         if (this.brain.getMemory(MemoryModuleType.ATTACK_TARGET).isEmpty() && var4 instanceof LivingEntity) {
            ...
            if (!(var1 instanceof IndirectEntityDamageSource) || this.closerThan(var5, 5.0)) {
   ...
   public boolean shouldListen(ServerLevel var1, GameEventListener var2, BlockPos var3, GameEvent var4, GameEvent.Context var5) {
      if (!this.isNoAi() && !this.isDeadOrDying() && !this.getBrain().hasMemoryValue(MemoryModuleType.VIBRATION_COOLDOWN) && !this.isDiggingOrEmerging() && var1.getWorldBorder().isWithinBounds(var3) && !this.isRemoved() && this.level == var1) {
         Entity var7 = var5.sourceEntity();
         boolean var10000;
         if (var7 instanceof LivingEntity) {
            LivingEntity var6 = (LivingEntity)var7;
            if (!this.canTargetEntity(var6)) {
               var10000 = false;
               return var10000;
            }
         }
   ...
   public void onSignalReceive(ServerLevel var1, GameEventListener var2, BlockPos var3, GameEvent var4, @Nullable Entity var5, @Nullable Entity var6, float var7) {
      if (!this.isDeadOrDying()) {
         ...
         BlockPos var8 = var3;
         if (var6 != null) {
            if (this.closerThan(var6, 30.0)) {
               if (this.getBrain().hasMemoryValue(MemoryModuleType.RECENT_PROJECTILE)) {
                  if (this.canTargetEntity(var6)) {
                     var8 = var6.blockPosition();
                  }

                  this.increaseAngerAt(var6);
               } else {
                  this.increaseAngerAt(var6, 10, true);
               }
            }

            this.getBrain().setMemoryWithExpiry(MemoryModuleType.RECENT_PROJECTILE, Unit.INSTANCE, 100L);
         } else {
            this.increaseAngerAt(var5);
         }

         if (!this.getAngerLevel().isAngry()) {
            Optional var9 = this.angerManagement.getActiveEntity();
            if (var6 != null || var9.isEmpty() || var9.get() == var5) {
               WardenAi.setDisturbanceLocation(this, var8);
            }
         }
