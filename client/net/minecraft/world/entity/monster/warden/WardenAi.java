...
public class WardenAi {
   ...
   private static final Behavior<Warden> DIG_COOLDOWN_SETTER;
   ...
   private static void initCoreActivity(Brain<Warden> var0) {
      var0.addActivity(Activity.CORE, 0, ImmutableList.of(new Swim(0.8F), new SetWardenLookTarget(), new LookAtTargetSink(45, 90), new MoveToTargetSink()));
   }
   ...
   private static void initIdleActivity(Brain<Warden> var0) {
      var0.addActivity(Activity.IDLE, 10, ImmutableList.of(new SetRoarTarget(Warden::getEntityAngryAt), new TryToSniff(), new RunOne(ImmutableMap.of(MemoryModuleType.IS_SNIFFING, MemoryStatus.VALUE_ABSENT), ImmutableList.of(Pair.of(new RandomStroll(0.5F), 2), Pair.of(new DoNothing(30, 60), 1)))));
   }

   private static void initInvestigateActivity(Brain<Warden> var0) {
      var0.addActivityAndRemoveMemoryWhenStopped(Activity.INVESTIGATE, 5, ImmutableList.of(new SetRoarTarget(Warden::getEntityAngryAt), new GoToTargetLocation(MemoryModuleType.DISTURBANCE_LOCATION, 2, 0.7F)), MemoryModuleType.DISTURBANCE_LOCATION);
   }

   private static void initSniffingActivity(Brain<Warden> var0) {
      var0.addActivityAndRemoveMemoryWhenStopped(Activity.SNIFF, 5, ImmutableList.of(new SetRoarTarget(Warden::getEntityAngryAt), new Sniffing(SNIFFING_DURATION)), MemoryModuleType.IS_SNIFFING);
   }
   ...
   private static void initFightActivity(Warden var0, Brain<Warden> var1) {
      var1.addActivityAndRemoveMemoryWhenStopped(Activity.FIGHT, 10, ImmutableList.of(DIG_COOLDOWN_SETTER, new StopAttackingIfTargetInvalid((var1x) -> {
         return !var0.getAngerLevel().isAngry() || !var0.canTargetEntity(var1x);
      }, WardenAi::onTargetInvalid, false), new SetEntityLookTarget((var1x) -> {
         return isTarget(var0, var1x);
      }, (float)var0.getAttributeValue(Attributes.FOLLOW_RANGE)), new SetWalkTargetFromAttackTargetIfTargetOutOfReach(1.2F), new SonicBoom(), new MeleeAttack(18)), MemoryModuleType.ATTACK_TARGET);
   }
   ...
   static {
      ...
      DIG_COOLDOWN_SETTER = new Behavior<Warden>(ImmutableMap.of(MemoryModuleType.DIG_COOLDOWN, MemoryStatus.REGISTERED)) {
         protected void start(ServerLevel var1, Warden var2, long var3) {
            WardenAi.setDigCooldown(var2);
         }
      };
   }
}
