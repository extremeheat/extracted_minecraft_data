...
public class YieldJobSite {
   ...
   public static BehaviorControl<Villager> create(float var0) {
      return BehaviorBuilder.create(
         var1 -> var1.group(
                  var1.present(MemoryModuleType.POTENTIAL_JOB_SITE),
                  var1.absent(MemoryModuleType.JOB_SITE),
                  var1.present(MemoryModuleType.NEAREST_LIVING_ENTITIES),
                  var1.registered(MemoryModuleType.WALK_TARGET),
                  ...
                  (var2, var3, var4, var5, var6) -> (var6x, var7, var8) -> {
                        if (var7.isBaby()) {
                           ...
                           if (var11.isEmpty()) {
                              ...
                              var1.<List<LivingEntity>>get(var4)
                                 .stream()
                                 .filter(var1xxx -> var1xxx instanceof Villager && var1xxx != var7)
                                 .map(var0xxxx -> (Villager)var0xxxx)
                                 .filter(LivingEntity::isAlive)
                                 .filter(var2xx -> nearbyWantsJobsite((Holder<PoiType>)var11.get(), var2xx, var10))
                                 .findFirst()
                                 .ifPresent(var6xx -> {
                                    var5.erase();
                                    var6.erase();
                                    ...
                                    if (var6xx.getBrain().getMemory(MemoryModuleType.JOB_SITE).isEmpty()) {
                                       BehaviorUtils.setWalkAndLookTargetMemories(var6xx, var10, var0, 1);
