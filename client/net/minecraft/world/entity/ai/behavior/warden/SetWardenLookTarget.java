...
public class SetWardenLookTarget extends Behavior<Warden> {
   ...
   protected void start(ServerLevel var1, Warden var2, long var3) {
      BlockPos var5 = (BlockPos)var2.getBrain().getMemory(MemoryModuleType.ROAR_TARGET).map(Entity::blockPosition).or(() -> {
         return var2.getBrain().getMemory(MemoryModuleType.DISTURBANCE_LOCATION);
      }).get();
      var2.getBrain().setMemory(MemoryModuleType.LOOK_TARGET, (Object)(new BlockPosTracker(var5)));
   }
}
