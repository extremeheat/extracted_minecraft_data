...
public class StageThree<E extends Warden> extends Behavior<E> {
   final Vec3 pathfindingOffset;
   final Pose pose;
   final float WALK_SPEED_MODIFIER = 0.7F;
   final int NEXT_STAGE = 4;

   public StageThree(int var1, Vec3 var2, Pose var3) {
   ...
   protected boolean checkExtraStartConditions(ServerLevel var1, E var2) {
      return !var2.getBrain().hasMemoryValue(MemoryModuleType.ATTACK_TARGET) && var2.getBrain().getMemory(MemoryModuleType.ACTING_STAGE).isPresent() && (Integer)var2.getBrain().getMemory(MemoryModuleType.ACTING_STAGE).get() == 3;
   }
   ...
   protected void stop(ServerLevel var1, E var2, long var3) {
      if (this.stageCleared(var2)) {
         ...
         var2.getBrain().setMemory(MemoryModuleType.ACTING_STAGE, 4);
      } else {
         var2.stopActing();
      }
   ...
   boolean stageCleared(E var1) {
      return var1.getBrain().getMemory(MemoryModuleType.ACTING_STAGE).isPresent() && (Integer)var1.getBrain().getMemory(MemoryModuleType.ACTING_STAGE).get() == 4;
   }
