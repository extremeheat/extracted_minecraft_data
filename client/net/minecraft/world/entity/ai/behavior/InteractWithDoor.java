...
public class InteractWithDoor extends Behavior<LivingEntity> {
   ...
   protected boolean checkExtraStartConditions(ServerLevel var1, LivingEntity var2) {
      Path var3 = var2.getBrain().getMemory(MemoryModuleType.PATH).get();
   ...
   protected void start(ServerLevel var1, LivingEntity var2, long var3) {
      Path var5 = var2.getBrain().getMemory(MemoryModuleType.PATH).get();
      ...
      if (var9.is(BlockTags.WOODEN_DOORS, var0 -> var0.getBlock() instanceof DoorBlock)) {
      ...
      if (var11.is(BlockTags.WOODEN_DOORS, var0 -> var0.getBlock() instanceof DoorBlock)) {
   ...
   public static void closeDoorsThatIHaveOpenedOrPassedThrough(ServerLevel var0, LivingEntity var1, @Nullable Node var2, @Nullable Node var3) {
      ...
      if (var4.hasMemoryValue(MemoryModuleType.DOORS_TO_CLOSE)) {
         Iterator var5 = var4.getMemory(MemoryModuleType.DOORS_TO_CLOSE).get().iterator();

         while(var5.hasNext()) {
            GlobalPos var6 = (GlobalPos)var5.next();
            BlockPos var7 = var6.pos();
            if ((var2 == null || !var2.asBlockPos().equals(var7)) && (var3 == null || !var3.asBlockPos().equals(var7))) {
               if (isDoorTooFarAway(var0, var1, var6)) {
                  var5.remove();
               } else {
                  BlockState var8 = var0.getBlockState(var7);
                  if (!var8.is(BlockTags.WOODEN_DOORS, var0x -> var0x.getBlock() instanceof DoorBlock)) {
                     var5.remove();
                  } else {
   ...
   private static boolean areOtherMobsComingThroughDoor(ServerLevel var0, LivingEntity var1, BlockPos var2) {
      ...
      return !var3.hasMemoryValue(MemoryModuleType.NEAREST_LIVING_ENTITIES)
         ? false
         : var3.getMemory(MemoryModuleType.NEAREST_LIVING_ENTITIES)
            .get()
            .stream()
            .filter(var1x -> var1x.getType() == var1.getType())
            .filter(var1x -> var2.closerToCenterThan(var1x.position(), 2.0))
            .anyMatch(var2x -> isMobComingThroughDoor(var0, var2x, var2));
   }
   ...
   private static boolean isMobComingThroughDoor(ServerLevel var0, LivingEntity var1, BlockPos var2) {
      if (!var1.getBrain().hasMemoryValue(MemoryModuleType.PATH)) {
         ...
         Path var3 = var1.getBrain().getMemory(MemoryModuleType.PATH).get();
         if (var3.isDone()) {
            return false;
         } else {
   ...
   private void rememberDoorToClose(ServerLevel var1, LivingEntity var2, BlockPos var3) {
      ...
      if (var4.getMemory(MemoryModuleType.DOORS_TO_CLOSE).isPresent()) {
         var4.getMemory(MemoryModuleType.DOORS_TO_CLOSE).get().add(var5);
      } else {
         var4.setMemory(MemoryModuleType.DOORS_TO_CLOSE, Sets.newHashSet(new GlobalPos[]{var5}));
      }
   }
}
