...
public class GossipContainer {
   ...
   public void update(Dynamic<?> var1) {
      var1.asStream().map(GossipEntry::load).flatMap((var0) -> {
         return var0.result().stream();
      }).forEach((var1x) -> {
   ...
   static class GossipEntry {
      ...
      public <T> Dynamic<T> store(DynamicOps<T> var1) {
         return new Dynamic(var1, var1.createMap(ImmutableMap.of(var1.createString("Target"), UUIDUtil.CODEC.encodeStart(var1, this.target).result().orElseThrow(RuntimeException::new), var1.createString("Type"), var1.createString(this.type.id), var1.createString("Value"), var1.createInt(this.value))));
      }

      public static DataResult<GossipEntry> load(Dynamic<?> var0) {
         return DataResult.unbox(DataResult.instance().group(var0.get("Target").read(UUIDUtil.CODEC), var0.get("Type").asString().map(GossipType::byId), var0.get("Value").asNumber().map(Number::intValue)).apply(DataResult.instance(), GossipEntry::new));
      }
   }
}
