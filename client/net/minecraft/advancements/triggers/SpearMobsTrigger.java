...
public class SpearMobsTrigger extends SimpleCriterionTrigger<TriggerInstance> {
   ...
   public static record TriggerInstance(Optional<Holder<LootItemCondition>> player, Optional<Integer> count) implements SimpleCriterionTrigger.SimpleInstance {
      public static final Codec<TriggerInstance> CODEC = RecordCodecBuilder.create((i) -> i.group(LootItemCondition.CODEC.optionalFieldOf("player").forGetter(TriggerInstance::player), ExtraCodecs.POSITIVE_INT.optionalFieldOf("count").forGetter(TriggerInstance::count)).apply(i, TriggerInstance::new));
