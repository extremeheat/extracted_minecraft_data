...
public class Bootstrap {
   ...
   private static void validateThatAllBiomeFeaturesHaveBiomeFilter() {
      BuiltinRegistries.BIOME.stream().forEach((var0) -> {
         ...
         var1.stream().flatMap(Collection::stream).forEach((var0x) -> {
            if (!((PlacedFeature)var0x.get()).getPlacement().contains(BiomeFilter.biome())) {
               Registry var10000 = BuiltinRegistries.PLACED_FEATURE;
               Util.logAndPauseIfInIde("Placed feature " + var10000.getResourceKey((PlacedFeature)var0x.get()) + " is missing BiomeFilter.biome()");
            }
