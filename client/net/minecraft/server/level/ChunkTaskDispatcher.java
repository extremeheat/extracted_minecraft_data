...
public class ChunkTaskDispatcher implements ChunkHolder.LevelChangeListener, AutoCloseable {
   ...
   protected void scheduleForExecution(ChunkTaskPriorityQueue.TasksForChunk var1) {
      CompletableFuture.allOf(var1.tasks().stream().map(var1x -> this.executor.scheduleWithResult(var1xx -> {
            var1x.run();
            var1xx.complete(Unit.INSTANCE);
         })).toArray(CompletableFuture[]::new)).thenAccept(var1x -> this.pollTask());
   }
