...
public class LocateCommand {
         ...
         (LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)Commands.literal("locate")
                           ...
                           Commands.argument("structure", ResourceOrTagKeyArgument.resourceOrTagKey(Registries.STRUCTURE))
                              .executes(
                                 var0x -> locateStructure(
                                       ...
                                       ResourceOrTagKeyArgument.getResourceOrTagKey(var0x, "structure", Registries.STRUCTURE, ERROR_STRUCTURE_INVALID)
                                    )
                              )
                        )
                  ))
               .then(
                  Commands.literal("biome")
                     .then(
                        Commands.argument("biome", ResourceOrTagArgument.resourceOrTag(var1, Registries.BIOME))
                           .executes(
                              var0x -> locateBiome(
                                    (CommandSourceStack)var0x.getSource(), ResourceOrTagArgument.getResourceOrTag(var0x, "biome", Registries.BIOME)
                                 )
                           )
                     )
               ))
   ...
   private static Optional<? extends HolderSet.ListBacked<Structure>> getHolders(ResourceOrTagKeyArgument.Result<Structure> var0, Registry<Structure> var1) {
      return (Optional<? extends HolderSet.ListBacked<Structure>>)var0.unwrap()
         .map(var1x -> var1.get(var1x).map(var0xx -> HolderSet.direct(var0xx)), var1::get);
   }
   ...
   public static int showLocateResult(
      CommandSourceStack var0,
      ResourceOrTagKeyArgument.Result<?> var1,
      BlockPos var2,
      Pair<BlockPos, ? extends Holder<?>> var3,
      String var4,
      boolean var5,
   ...
   private static int showLocateResult(
      CommandSourceStack var0, BlockPos var1, Pair<BlockPos, ? extends Holder<?>> var2, String var3, boolean var4, String var5, Duration var6
   ) {
      ...
      MutableComponent var10 = ComponentUtils.wrapInSquareBrackets(Component.translatable("chat.coordinates", var7.getX(), var9, var7.getZ()))
         .withStyle(
            var2x -> var2x.withColor(ChatFormatting.GREEN)
                  .withClickEvent(new ClickEvent(ClickEvent.Action.SUGGEST_COMMAND, "/tp @s " + var7.getX() + " " + var9 + " " + var7.getZ()))
