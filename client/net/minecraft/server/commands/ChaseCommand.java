...
public class ChaseCommand {
   ...
   private static int stop(CommandSourceStack var0) {
      if (chaseClient != null) {
         chaseClient.stop();
         var0.sendSuccess(new TextComponent("You have now stopped chasing"), false);
         chaseClient = null;
      }

      if (chaseServer != null) {
         chaseServer.stop();
         var0.sendSuccess(new TextComponent("You are no longer being chased"), false);
         chaseServer = null;
      }
   ...
   private static boolean alreadyRunning(CommandSourceStack var0) {
      if (chaseServer != null) {
         var0.sendFailure(new TextComponent("Chase server is already running. Stop it using /chase stop"));
         return true;
      } else if (chaseClient != null) {
         var0.sendFailure(new TextComponent("You are already chasing someone. Stop it using /chase stop"));
         return true;
      } else {
         return false;
      }
   ...
   private static int lead(CommandSourceStack var0, String var1, int var2) {
      if (alreadyRunning(var0)) {
         ...
         try {
            chaseServer.start();
            var0.sendSuccess(new TextComponent("Chase server is now running on port " + var2 + ". Clients can follow you using /chase follow <ip> <port>"), false);
         } catch (IOException var4) {
            var4.printStackTrace();
            var0.sendFailure(new TextComponent("Failed to start chase server on port " + var2));
            chaseServer = null;
         }
   ...
   private static int follow(CommandSourceStack var0, String var1, int var2) {
      if (alreadyRunning(var0)) {
         ...
         chaseClient.start();
         var0.sendSuccess(new TextComponent("You are now chasing " + var1 + ":" + var2 + ". If that server does '/chase lead' then you will automatically go to the same position. Use '/chase stop' to stop chasing."), false);
         return 0;
      }
   }
