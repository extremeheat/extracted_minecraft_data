...
public class RandomCommand {
   ...
   public static void register(CommandDispatcher<CommandSourceStack> var0) {
      var0.register(
         (LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)Commands.literal("random").then(drawRandomValueTree("value", false)))
            ...
            .then(
               ((LiteralArgumentBuilder)((LiteralArgumentBuilder)Commands.literal("reset").requires(var0x -> var0x.hasPermission(2)))
                     .then(
                        ((LiteralArgumentBuilder)Commands.literal("*").executes(var0x -> resetAllSequences((CommandSourceStack)var0x.getSource())))
                           .then(
                              ((RequiredArgumentBuilder)Commands.argument("seed", IntegerArgumentType.integer())
                                    .executes(
                                       var0x -> resetAllSequencesAndSetNewDefaults(
                                             (CommandSourceStack)var0x.getSource(), IntegerArgumentType.getInteger(var0x, "seed"), true, true
                                          )
                                    ))
                                 .then(
                                    ((RequiredArgumentBuilder)Commands.argument("includeWorldSeed", BoolArgumentType.bool())
                                          .executes(
                                             var0x -> resetAllSequencesAndSetNewDefaults(
                                                   ...
                                                   BoolArgumentType.getBool(var0x, "includeWorldSeed"),
                                                   true
                                                )
                                          ))
                                       .then(
                                          Commands.argument("includeSequenceId", BoolArgumentType.bool())
                                             .executes(
                                                var0x -> resetAllSequencesAndSetNewDefaults(
                                                      ...
                                                      IntegerArgumentType.getInteger(var0x, "seed"),
                                                      BoolArgumentType.getBool(var0x, "includeWorldSeed"),
                                                      BoolArgumentType.getBool(var0x, "includeSequenceId")
                                                   )
                                             )
                                       )
                                 )
                  ...
                  .then(
                     ((RequiredArgumentBuilder)Commands.argument("sequence", ResourceLocationArgument.id())
                           .suggests(RandomCommand::suggestRandomSequence)
                           .executes(var0x -> resetSequence((CommandSourceStack)var0x.getSource(), ResourceLocationArgument.getId(var0x, "sequence"))))
                        .then(
                           ((RequiredArgumentBuilder)Commands.argument("seed", IntegerArgumentType.integer())
                                 .executes(
                                    var0x -> resetSequence(
                                          (CommandSourceStack)var0x.getSource(),
                                          ResourceLocationArgument.getId(var0x, "sequence"),
                                          IntegerArgumentType.getInteger(var0x, "seed"),
                                          true,
                                          true
                                       )
      ...
      return (LiteralArgumentBuilder<CommandSourceStack>)Commands.literal(var0)
         .then(
            ((RequiredArgumentBuilder)Commands.argument("range", RangeArgument.intRange())
                  .executes(var1x -> randomSample((CommandSourceStack)var1x.getSource(), RangeArgument.Ints.getRange(var1x, "range"), null, var1)))
               .then(
                  ((RequiredArgumentBuilder)Commands.argument("sequence", ResourceLocationArgument.id())
                        .suggests(RandomCommand::suggestRandomSequence)
                     ...
                     .executes(
                        var1x -> randomSample(
                              ...
                              RangeArgument.Ints.getRange(var1x, "range"),
                              ResourceLocationArgument.getId(var1x, "sequence"),
                              var1
                           )
                     )
               )
   ...
   private static int randomSample(CommandSourceStack var0, MinMaxBounds.Ints var1, @Nullable ResourceLocation var2, boolean var3) throws CommandSyntaxException {
      ...
      int var5 = var1.min().orElse(-2147483648);
      int var6 = var1.max().orElse(2147483647);
      ...
      if (var7 == 0L) {
         ...
         if (var3) {
            var0.getServer()
               .getPlayerList()
               .broadcastSystemMessage(Component.translatable("commands.random.roll", var0.getDisplayName(), var9, var5, var6), false);
         } else {
