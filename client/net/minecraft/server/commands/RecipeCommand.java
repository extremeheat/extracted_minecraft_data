...
public class RecipeCommand {
   ...
   public static void register(CommandDispatcher<CommandSourceStack> var0) {
      ...
      })).then(Commands.literal("give").then(((RequiredArgumentBuilder)Commands.argument("targets", EntityArgument.players()).then(Commands.argument("recipe", ResourceLocationArgument.method_50()).suggests(SuggestionProviders.ALL_RECIPES).executes((var0x) -> {
      ...
      }))))).then(Commands.literal("take").then(((RequiredArgumentBuilder)Commands.argument("targets", EntityArgument.players()).then(Commands.argument("recipe", ResourceLocationArgument.method_50()).suggests(SuggestionProviders.ALL_RECIPES).executes((var0x) -> {
   ...
   private static int giveRecipes(CommandSourceStack var0, Collection<ServerPlayer> var1, Collection<Recipe<?>> var2) throws CommandSyntaxException {
      ...
      if (var3 == 0) {
         ...
         if (var1.size() == 1) {
            var0.sendSuccess(new TranslatableComponent("commands.recipe.give.success.single", new Object[]{var2.size(), ((ServerPlayer)var1.iterator().next()).getDisplayName()}), true);
         } else {
            var0.sendSuccess(new TranslatableComponent("commands.recipe.give.success.multiple", new Object[]{var2.size(), var1.size()}), true);
         }
   ...
   private static int takeRecipes(CommandSourceStack var0, Collection<ServerPlayer> var1, Collection<Recipe<?>> var2) throws CommandSyntaxException {
      ...
      if (var3 == 0) {
         ...
         if (var1.size() == 1) {
            var0.sendSuccess(new TranslatableComponent("commands.recipe.take.success.single", new Object[]{var2.size(), ((ServerPlayer)var1.iterator().next()).getDisplayName()}), true);
         } else {
            var0.sendSuccess(new TranslatableComponent("commands.recipe.take.success.multiple", new Object[]{var2.size(), var1.size()}), true);
         }
