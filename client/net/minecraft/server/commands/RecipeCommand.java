...
public class RecipeCommand {
   ...
   public static void register(CommandDispatcher<CommandSourceStack> var0) {
      var0.register(
         (LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)Commands.literal("recipe").requires(var0x -> var0x.hasPermission(2)))
               .then(
                              ...
                              Commands.literal("*")
                                 .executes(
                                    var0x -> giveRecipes(
                                          (CommandSourceStack)var0x.getSource(),
                                          EntityArgument.getPlayers(var0x, "targets"),
                                          ((CommandSourceStack)var0x.getSource()).getServer().getRecipeManager().getRecipes()
                                       )
                                 )
                           )
                     )
   ...
   private static int giveRecipes(CommandSourceStack var0, Collection<ServerPlayer> var1, Collection<Recipe<?>> var2) throws CommandSyntaxException {
      ...
      if (var3 == 0) {
         ...
         if (var1.size() == 1) {
            var0.sendSuccess(
               () -> Component.translatable("commands.recipe.give.success.single", var2.size(), ((ServerPlayer)var1.iterator().next()).getDisplayName()), true
            );
         } else {
   ...
   private static int takeRecipes(CommandSourceStack var0, Collection<ServerPlayer> var1, Collection<Recipe<?>> var2) throws CommandSyntaxException {
      ...
      if (var3 == 0) {
         ...
         if (var1.size() == 1) {
            var0.sendSuccess(
               () -> Component.translatable("commands.recipe.take.success.single", var2.size(), ((ServerPlayer)var1.iterator().next()).getDisplayName()), true
            );
         } else {
