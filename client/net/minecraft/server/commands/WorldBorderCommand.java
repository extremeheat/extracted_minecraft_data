...
public class WorldBorderCommand {
   ...
   private static int setWarningTime(CommandSourceStack var0, int var1) throws CommandSyntaxException {
      ...
      if (var2.getWarningTime() == var1) {
         ...
         var0.sendSuccess(() -> {
            return Component.translatable("commands.worldborder.warning.time.success", var1);
         }, true);
         return var1;
      }
   }
   ...
   private static int setSize(CommandSourceStack var0, double var1, long var3) throws CommandSyntaxException {
      ...
      if (var6 == var1) {
         ...
         if (var3 > 0L) {
            ...
            if (var1 > var6) {
               var0.sendSuccess(() -> {
                  return Component.translatable("commands.worldborder.set.grow", String.format(Locale.ROOT, "%.1f", var1), Long.toString(var3 / 1000L));
               }, true);
            } else {
               var0.sendSuccess(() -> {
                  return Component.translatable("commands.worldborder.set.shrink", String.format(Locale.ROOT, "%.1f", var1), Long.toString(var3 / 1000L));
               }, true);
            }
         } else {
            var5.setSize(var1);
            var0.sendSuccess(() -> {
               return Component.translatable("commands.worldborder.set.immediate", String.format(Locale.ROOT, "%.1f", var1));
            }, true);
         }
