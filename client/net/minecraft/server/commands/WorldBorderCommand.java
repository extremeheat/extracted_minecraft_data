...
public class WorldBorderCommand {
         ...
         (LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)Commands.literal(
                           ...
                           .then(
                              Commands.literal("add")
                                 .then(
                                    ((RequiredArgumentBuilder)Commands.argument("distance", DoubleArgumentType.doubleArg(-5.9999968E7, 5.9999968E7))
                                          .executes(
                                             var0x -> setSize(
                                                   (CommandSourceStack)var0x.getSource(),
                                                   ((CommandSourceStack)var0x.getSource()).getLevel().getWorldBorder().getSize()
                                          ...
                                          Commands.argument("time", IntegerArgumentType.integer(0))
                                             .executes(
                                                var0x -> setSize(
                                                      ...
                                                      ((CommandSourceStack)var0x.getSource()).getLevel().getWorldBorder().getSize()
                                                         + DoubleArgumentType.getDouble(var0x, "distance"),
                                                      ((CommandSourceStack)var0x.getSource()).getLevel().getWorldBorder().getLerpRemainingTime()
                        ...
                        .then(
                           Commands.literal("set")
                              .then(
                                 ((RequiredArgumentBuilder)Commands.argument("distance", DoubleArgumentType.doubleArg(-5.9999968E7, 5.9999968E7))
                                       .executes(var0x -> setSize((CommandSourceStack)var0x.getSource(), DoubleArgumentType.getDouble(var0x, "distance"), 0L)))
                                    .then(
                                       Commands.argument("time", IntegerArgumentType.integer(0))
                                          .executes(
                                             var0x -> setSize(
                        ...
                        Commands.literal("center")
                           .then(
                              Commands.argument("pos", Vec2Argument.vec2())
                                 .executes(var0x -> setCenter((CommandSourceStack)var0x.getSource(), Vec2Argument.getVec2(var0x, "pos")))
                           )
                     ))
                  .then(
                              ...
                              Commands.literal("amount")
                                 .then(
                                    Commands.argument("damagePerBlock", FloatArgumentType.floatArg(0.0F))
                                       .executes(
                                          var0x -> setDamageAmount((CommandSourceStack)var0x.getSource(), FloatArgumentType.getFloat(var0x, "damagePerBlock"))
                                       )
                                 )
                           ))
                        .then(
                           Commands.literal("buffer")
                              .then(
                                 Commands.argument("distance", FloatArgumentType.floatArg(0.0F))
                                    .executes(var0x -> setDamageBuffer((CommandSourceStack)var0x.getSource(), FloatArgumentType.getFloat(var0x, "distance")))
                              )
                        )
                  ))
               .then(Commands.literal("get").executes(var0x -> getSize((CommandSourceStack)var0x.getSource()))))
            .then(
                        ...
                        Commands.literal("distance")
                           .then(
                              Commands.argument("distance", IntegerArgumentType.integer(0))
                                 .executes(
                                    var0x -> setWarningDistance((CommandSourceStack)var0x.getSource(), IntegerArgumentType.getInteger(var0x, "distance"))
                                 )
                           )
                     ))
                  .then(
                     Commands.literal("time")
                        .then(
                           Commands.argument("time", IntegerArgumentType.integer(0))
                              .executes(var0x -> setWarningTime((CommandSourceStack)var0x.getSource(), IntegerArgumentType.getInteger(var0x, "time")))
                        )
                  )
            )
      );
   ...
   private static int setCenter(CommandSourceStack var0, Vec2 var1) throws CommandSyntaxException {
      ...
      if (var2.getCenterX() == (double)var1.x && var2.getCenterZ() == (double)var1.y) {
         ...
         var0.sendSuccess(
            Component.translatable("commands.worldborder.center.success", String.format(Locale.ROOT, "%.2f", var1.x), String.format("%.2f", var1.y)), true
         );
         return 0;
      } else {
