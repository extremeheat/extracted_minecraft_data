...
public class EnchantCommand {
   ...
   public static void register(CommandDispatcher<CommandSourceStack> var0, CommandBuildContext var1) {
      var0.register((LiteralArgumentBuilder)((LiteralArgumentBuilder)Commands.literal("enchant").requires(Commands.hasPermission(2))).then(Commands.argument("targets", EntityArgument.entities()).then(((RequiredArgumentBuilder)Commands.argument("enchantment", ResourceArgument.resource(var1, Registries.ENCHANTMENT)).executes((var0x) -> enchant((CommandSourceStack)var0x.getSource(), EntityArgument.getEntities(var0x, "targets"), ResourceArgument.getEnchantment(var0x, "enchantment"), 1))).then(Commands.argument("level", IntegerArgumentType.integer(0)).executes((var0x) -> enchant((CommandSourceStack)var0x.getSource(), EntityArgument.getEntities(var0x, "targets"), ResourceArgument.getEnchantment(var0x, "enchantment"), IntegerArgumentType.getInteger(var0x, "level")))))));
   }
   ...
   private static int enchant(CommandSourceStack var0, Collection<? extends Entity> var1, Holder<Enchantment> var2, int var3) throws CommandSyntaxException {
      ...
      if (var3 > var4.getMaxLevel()) {
         ...
         if (var5 == 0) {
            ...
            if (var1.size() == 1) {
               var0.sendSuccess(() -> Component.translatable("commands.enchant.success.single", Enchantment.getFullname(var2, var3), ((Entity)var1.iterator().next()).getDisplayName()), true);
            } else {
