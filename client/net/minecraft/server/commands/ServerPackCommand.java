...
public class ServerPackCommand {
   ...
   public static void register(CommandDispatcher<CommandSourceStack> var0) {
      var0.register(
         (LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)Commands.literal("serverpack").requires(var0x -> var0x.hasPermission(2)))
               .then(
                  Commands.literal("push")
                     .then(
                        ((RequiredArgumentBuilder)Commands.argument("url", StringArgumentType.string())
                                          ...
                                          Commands.argument("hash", StringArgumentType.word())
                                             .executes(
                                                var0x -> pushPack(
                                                      ...
                                                      StringArgumentType.getString(var0x, "url"),
                                                      Optional.of(UuidArgument.getUuid(var0x, "uuid")),
                                                      Optional.of(StringArgumentType.getString(var0x, "hash"))
                                                   )
                                             )
                                       ))
                                    .executes(
                           ...
                           .executes(
                              var0x -> pushPack(
                                    (CommandSourceStack)var0x.getSource(), StringArgumentType.getString(var0x, "url"), Optional.empty(), Optional.empty()
                                 )
                           )
                     )
               ))
               ...
               Commands.literal("pop")
                  .then(
                     Commands.argument("uuid", UuidArgument.uuid())
                        .executes(var0x -> popPack((CommandSourceStack)var0x.getSource(), UuidArgument.getUuid(var0x, "uuid")))
                  )
            )
      );
   }
