   ...
   public static void register(CommandDispatcher<CommandSourceStack> var0) {
      var0.register(
         (LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)Commands.literal(
                           ...
                           .then(Commands.literal("on").executes(var0x -> enableWhitelist((CommandSourceStack)var0x.getSource()))))
                        .then(Commands.literal("off").executes(var0x -> disableWhitelist((CommandSourceStack)var0x.getSource()))))
                     .then(Commands.literal("list").executes(var0x -> showList((CommandSourceStack)var0x.getSource()))))
                  .then(
                              ...
                              .suggests(
                                 (var0x, var1) -> {
                                    ...
                                    return SharedSuggestionProvider.suggest(
                                       var2.getPlayers()
                                          .stream()
                                          .filter(var1x -> !var2.getWhiteList().isWhiteListed(var1x.getGameProfile()))
                                          .map(var0xx -> var0xx.getGameProfile().getName()),
                                       var1
                                    );
                                 }
                              )
                              .executes(var0x -> addPlayers((CommandSourceStack)var0x.getSource(), GameProfileArgument.getGameProfiles(var0x, "targets")))
                        )
                  ))
               .then(
                        ...
                        Commands.argument("targets", GameProfileArgument.gameProfile())
                           .suggests(
                              (var0x, var1) -> SharedSuggestionProvider.suggest(
                                    ((CommandSourceStack)var0x.getSource()).getServer().getPlayerList().getWhiteListNames(), var1
                                 )
                           )
                           .executes(var0x -> removePlayers((CommandSourceStack)var0x.getSource(), GameProfileArgument.getGameProfiles(var0x, "targets")))
                     )
               ))
            .then(Commands.literal("reload").executes(var0x -> reload((CommandSourceStack)var0x.getSource())))
      );
   }
