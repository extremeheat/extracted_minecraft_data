...
public class PublishCommand {
   ...
   private static final DynamicCommandExceptionType ERROR_ALREADY_PUBLISHED = new DynamicCommandExceptionType(
      var0 -> Component.translatableEscape("commands.publish.alreadyPublished", var0)
   );
   ...
   public static void register(CommandDispatcher<CommandSourceStack> var0) {
      var0.register(
         (LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)Commands.literal("publish").requires(var0x -> var0x.hasPermission(4)))
               .executes(var0x -> publish((CommandSourceStack)var0x.getSource(), HttpUtil.getAvailablePort(), false, null)))
            .then(
               ((RequiredArgumentBuilder)Commands.argument("allowCommands", BoolArgumentType.bool())
                     .executes(
                        var0x -> publish(
                              (CommandSourceStack)var0x.getSource(), HttpUtil.getAvailablePort(), BoolArgumentType.getBool(var0x, "allowCommands"), null
                           )
                     ))
                  .then(
