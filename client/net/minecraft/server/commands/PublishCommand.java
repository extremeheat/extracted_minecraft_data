...
public class PublishCommand {
   ...
   private static final DynamicCommandExceptionType ERROR_ALREADY_PUBLISHED = new DynamicCommandExceptionType(
      var0 -> Component.translatable("commands.publish.alreadyPublished", var0)
   );
   ...
   public static void register(CommandDispatcher<CommandSourceStack> var0) {
      var0.register(
         (LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)Commands.literal("publish").requires(var0x -> var0x.hasPermission(4)))
               .executes(var0x -> publish((CommandSourceStack)var0x.getSource(), HttpUtil.getAvailablePort())))
            .then(
               Commands.argument("port", IntegerArgumentType.integer(0, 65535))
                  .executes(var0x -> publish((CommandSourceStack)var0x.getSource(), IntegerArgumentType.getInteger(var0x, "port")))
            )
      );
   }
   ...
   private static int publish(CommandSourceStack var0, int var1) throws CommandSyntaxException {
      ...
      } else if (!var0.getServer().publishServer(null, false, var1)) {
