...
public class JfrCommand {
   ...
   private static final DynamicCommandExceptionType DUMP_FAILED = new DynamicCommandExceptionType((var0) -> {
      return new TranslatableComponent("commands.jfr.dump.failed", new Object[]{var0});
   });
   ...
   private static int startJfr(CommandSourceStack var0) throws CommandSyntaxException {
      ...
      if (!JvmProfiler.INSTANCE.start(var1)) {
         ...
         var0.sendSuccess(new TranslatableComponent("commands.jfr.started"), false);
         return 1;
      }
   }
   ...
   private static int stopJfr(CommandSourceStack var0) throws CommandSyntaxException {
      try {
         ...
         MutableComponent var3 = (new TextComponent(var1.toString())).withStyle(ChatFormatting.UNDERLINE).withStyle((var1x) -> {
            return var1x.withClickEvent(new ClickEvent(ClickEvent.Action.COPY_TO_CLIPBOARD, var2.toString())).withHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, new TranslatableComponent("chat.copy.click")));
         });
         var0.sendSuccess(new TranslatableComponent("commands.jfr.stopped", new Object[]{var3}), false);
         return 1;
