...
public class TagCommand {
   ...
   public static void register(CommandDispatcher<CommandSourceStack> var0) {
      var0.register((LiteralArgumentBuilder)((LiteralArgumentBuilder)Commands.literal("tag").requires((var0x) -> var0x.hasPermission(2))).then(((RequiredArgumentBuilder)((RequiredArgumentBuilder)Commands.argument("targets", EntityArgument.entities()).then(Commands.literal("add").then(Commands.argument("name", StringArgumentType.word()).executes((var0x) -> addTag((CommandSourceStack)var0x.getSource(), EntityArgument.getEntities(var0x, "targets"), StringArgumentType.getString(var0x, "name")))))).then(Commands.literal("remove").then(Commands.argument("name", StringArgumentType.word()).suggests((var0x, var1) -> SharedSuggestionProvider.suggest(getTags(EntityArgument.getEntities(var0x, "targets")), var1)).executes((var0x) -> removeTag((CommandSourceStack)var0x.getSource(), EntityArgument.getEntities(var0x, "targets"), StringArgumentType.getString(var0x, "name")))))).then(Commands.literal("list").executes((var0x) -> listTags((CommandSourceStack)var0x.getSource(), EntityArgument.getEntities(var0x, "targets"))))));
   }
