   ...
   protected TypeRewriteRule makeRule() {
      return this.writeFixAndRead("Food to consumable fix", this.getInputSchema().getType(References.DATA_COMPONENTS), this.getOutputSchema().getType(References.DATA_COMPONENTS), (var0) -> {
         ...
         if (var1.isPresent()) {
            ...
            Stream var4 = var3.map((var0x) -> var0x.emptyMap().set("type", var0x.createString("minecraft:apply_effects")).set("effects", var0x.createList(var0x.get("effect").result().stream())).set("probability", var0x.createFloat(var0x.get("probability").asFloat(1.0F))));
