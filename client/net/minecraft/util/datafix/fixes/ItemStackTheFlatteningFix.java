...
public class ItemStackTheFlatteningFix extends DataFix {
   ...
   public ItemStackTheFlatteningFix(final Schema outputSchema, final boolean changesType) {
   ...
   public TypeRewriteRule makeRule() {
      ...
      return this.fixTypeEverywhereTyped("ItemInstanceTheFlatteningFix", itemStackType, (input) -> {
         ...
         if (id.isEmpty()) {
            ...
            int data = rest.get("Damage").asInt(0);
            String newValue = updateItem((String)((Pair)id.get()).getSecond(), data);
            if (newValue != null) {
               output = input.set(idF, Pair.of(References.ITEM_NAME.typeName(), newValue));
            }

            if (DAMAGE_IDS.contains(((Pair)id.get()).getSecond())) {
               Typed<?> tag = input.getOrCreateTyped(tagF);
               Dynamic<?> tagRest = (Dynamic)tag.get(DSL.remainderFinder());
               tagRest = tagRest.set("Damage", tagRest.createInt(data));
               output = output.set(tagF, tag.set(DSL.remainderFinder(), tagRest));
            }

            output = output.set(DSL.remainderFinder(), rest.remove("Damage"));
            return output;
         }
      });
   }

   public static @Nullable String updateItem(final @Nullable String name, final int data) {
      if (IDS.contains(name)) {
         String newName = (String)MAP.get(name + "." + data);
         return newName == null ? (String)MAP.get(name + ".0") : newName;
      } else {
         return null;
      }
   }
