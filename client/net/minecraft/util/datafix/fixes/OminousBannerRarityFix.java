...
public class OminousBannerRarityFix extends DataFix {
   ...
   public TypeRewriteRule makeRule() {
      ...
      return TypeRewriteRule.seq(this.fixTypeEverywhereTyped("Ominous Banner block entity common rarity to uncommon rarity fix", var1, var3x -> {
         Object var4x = ((Pair)var3x.get(var3.finder())).getFirst();
         return var4x.equals("minecraft:banner") ? this.fix(var3x, var5) : var3x;
      }), this.fixTypeEverywhereTyped("Ominous Banner item stack common rarity to uncommon rarity fix", var2, var3x -> {
         String var4x = var3x.getOptional(var4).<String>map(Pair::getSecond).orElse("");
   ...
   private Typed<?> fix(Typed<?> var1, OpticFinder<?> var2) {
      return var1.updateTyped(
         ...
         var0 -> var0.update(
               DSL.remainderFinder(),
               var0x -> {
                  boolean var1x = var0x.get("minecraft:item_name")
                     .asString()
                     .result()
                     .flatMap(ComponentDataFixUtils::extractTranslationString)
                     .filter(var0xx -> var0xx.equals("block.minecraft.ominous_banner"))
                     .isPresent();
                  return var1x
                     ? var0x.set("minecraft:rarity", var0x.createString("uncommon"))
                        .set("minecraft:item_name", ComponentDataFixUtils.createTranslatableComponent(var0x.getOps(), "block.minecraft.ominous_banner"))
                     : var0x;
               }
            )
      );
