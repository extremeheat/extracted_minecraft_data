...
public class ExplorerMapItemFix extends DataFix {
   ...
   public ExplorerMapItemFix(final Schema outputSchema) {
      super(outputSchema, false);
   }

   private static <T> Dynamic<T> removeLegacyItemName(final Dynamic<T> components, final String legacyNameKey) {
      boolean holdsLegacyName = components.get("minecraft:item_name").result().flatMap(ExtraDataFixUtils::getPlainTranslationKey).filter((nameKey) -> nameKey.equals(legacyNameKey)).isPresent();
      return holdsLegacyName ? components.remove("minecraft:item_name") : components;
   }
   ...
   public TypeRewriteRule makeRule() {
      ...
      return this.fixTypeEverywhereTyped("ExplorerMapItemFix", itemStackType, (itemStack) -> {
         ...
         if (id.filter((itemId) -> itemId.equals("minecraft:filled_map")).isEmpty()) {
            ...
            Optional<? extends Typed<?>> components = itemStack.getOptionalTyped(componentsFinder);
            if (components.isEmpty()) {
               return itemStack;
            } else {
               Dynamic<?> componentsData = (Dynamic)((Typed)components.get()).getOrCreate(DSL.remainderFinder());
               Optional<? extends Dynamic<?>> mapDecorations = componentsData.get("minecraft:map_decorations").result();
               if (mapDecorations.isEmpty()) {
                  return itemStack;
               } else {
                  Optional<? extends Dynamic<?>> explorerDecoration = ((Dynamic)mapDecorations.get()).get("+").result();
                  if (explorerDecoration.isEmpty()) {
                     ...
                     String newItemId = (String)DECORATION_TYPE_TO_ITEM_ID.get(decorationType);
                     if (newItemId == null) {
                        return itemStack;
                     } else {
