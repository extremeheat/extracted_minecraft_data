...
public class BannerEntityCustomNameToOverrideComponentFix extends DataFix {
   ...
   private Typed<?> fix(Typed<?> var1, OpticFinder<?> var2) {
      ...
      boolean var5 = var4.asString()
         .result()
         .flatMap(ComponentDataFixUtils::extractTranslationString)
         .filter(var0 -> var0.equals("block.minecraft.ominous_banner"))
         .isPresent();
      if (var5) {
         Typed var6 = var1.getOrCreateTyped(var2)
            .update(
               DSL.remainderFinder(),
               var1x -> var1x.set("minecraft:item_name", (Dynamic)var4.result().get()).set("minecraft:hide_additional_tooltip", var1x.createMap(Map.of()))
            );
