...
public class AttributeModifierIdFix extends DataFix {
   ...
   protected TypeRewriteRule makeRule() {
      ...
      return TypeRewriteRule.seq(
         this.fixTypeEverywhereTyped(
            ...
            var1,
            var1x -> var1x.updateTyped(var2, var0x -> var0x.update(DSL.remainderFinder(), AttributeModifierIdFix::fixItemStackComponents))
         ),
         new TypeRewriteRule[]{
   ...
   private static Stream<Dynamic<?>> fixModifiers(Stream<Dynamic<?>> var0) {
      ...
      var0.forEach(var1x -> {
         ...
         String var4 = var2 != null ? ID_MAP.get(var2) : null;
         String var5 = NAME_MAP.get(var3);
         if (var4 != null) {
            ...
            String var14 = "minecraft:" + (var2 != null ? var2.toString().toLowerCase(Locale.ROOT) : "unknown");
   ...
   private static Dynamic<?> convertModifierForEntity(Dynamic<?> var0) {
      return var0.renameField("UUID", "uuid")
         .renameField("Name", "name")
         .renameField("Amount", "amount")
         .renameAndFixField("Operation", "operation", var0x -> {
      ...
      return var0.update(
         ...
         var0x -> var0x.update(
               ...
               var0xx -> (Dynamic)DataFixUtils.orElse(
                     var0xx.asStreamOpt().result().map(AttributeModifierIdFix::fixModifiersTypeWrapper).map(var0xx::createList), var0xx
                  )
            )
      );
   }
   ...
   private static Dynamic<?> fixAttribute(Dynamic<?> var0) {
      return var0.renameField("Name", "id")
         ...
         .renameAndFixField(
            ...
            var1 -> (Dynamic)DataFixUtils.orElse(
                  var1.asStreamOpt()
                     .result()
                     .map(var0xx -> var0xx.map(AttributeModifierIdFix::convertModifierForEntity))
   ...
   private static Typed<?> fixEntity(Typed<?> var0) {
      return var0.update(
         DSL.remainderFinder(),
         var0x -> var0x.renameAndFixField(
               "Attributes",
               "attributes",
               var0xx -> (Dynamic)DataFixUtils.orElse(
                     var0xx.asStreamOpt().result().map(var0xxx -> var0xxx.map(AttributeModifierIdFix::fixAttribute)).map(var0xx::createList), var0xx
                  )
            )
      );
   }
