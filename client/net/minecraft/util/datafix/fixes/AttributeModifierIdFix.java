...
public class AttributeModifierIdFix extends DataFix {
   ...
   private static Stream<Dynamic<?>> fixModifiers(final Stream<Dynamic<?>> modifiers) {
      ...
      modifiers.forEach((modifier) -> {
         ...
         String idFromUUID = uuid != null ? (String)ID_MAP.get(uuid) : null;
         String idFromName = (String)NAME_MAP.get(name);
         if (idFromUUID != null) {
            ...
            String var10000 = uuid != null ? uuid.toString().toLowerCase(Locale.ROOT) : "unknown";
            String id = "minecraft:" + var10000;
   ...
   private static Dynamic<?> convertModifierForEntity(final Dynamic<?> modifier) {
      return modifier.renameField("UUID", "uuid").renameField("Name", "name").renameField("Amount", "amount").renameAndFixField("Operation", "operation", (operation) -> {
         String var10001;
   ...
   private static Dynamic<?> fixItemStackComponents(final Dynamic<?> components) {
      return components.update("minecraft:attribute_modifiers", (attributeModifiers) -> attributeModifiers.update("modifiers", (modifiers) -> {
            Optional var10000 = modifiers.asStreamOpt().result().map(AttributeModifierIdFix::fixModifiersTypeWrapper);
   ...
   private static Dynamic<?> fixAttribute(final Dynamic<?> attribute) {
      return attribute.renameField("Name", "id").renameField("Base", "base").renameAndFixField("Modifiers", "modifiers", (modifiers) -> {
         Optional var10000 = modifiers.asStreamOpt().result().map((s) -> s.map(AttributeModifierIdFix::convertModifierForEntity)).map(AttributeModifierIdFix::fixModifiersTypeWrapper);
         Objects.requireNonNull(attribute);
         return (Dynamic)DataFixUtils.orElse(var10000.map(attribute::createList), modifiers);
      });
   }

   private static Typed<?> fixEntity(final Typed<?> entity) {
      return entity.update(DSL.remainderFinder(), (tag) -> tag.renameAndFixField("Attributes", "attributes", (attributeList) -> {
            Optional var10000 = attributeList.asStreamOpt().result().map((s) -> s.map(AttributeModifierIdFix::fixAttribute));
   ...
   public static @Nullable UUID uuidFromIntArray(final int[] intArray) {
      return intArray.length != 4 ? null : new UUID((long)intArray[0] << 32 | (long)intArray[1] & 4294967295L, (long)intArray[2] << 32 | (long)intArray[3] & 4294967295L);
   }
}
