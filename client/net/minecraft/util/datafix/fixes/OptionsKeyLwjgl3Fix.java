...
public class OptionsKeyLwjgl3Fix extends DataFix {
   ...
   public OptionsKeyLwjgl3Fix(final Schema outputSchema, final boolean changesType) {
      super(outputSchema, changesType);
   }

   public TypeRewriteRule makeRule() {
      return this.fixTypeEverywhereTyped("OptionsKeyLwjgl3Fix", this.getInputSchema().getType(References.OPTIONS), (input) -> input.update(DSL.remainderFinder(), (tag) -> (Dynamic)tag.getMapValues().map((map1) -> tag.createMap((Map)map1.entrySet().stream().map((entry) -> {
                  if (((Dynamic)entry.getKey()).asString("").startsWith("key_")) {
                     int oldValue = Integer.parseInt(((Dynamic)entry.getValue()).asString(""));
                     if (oldValue < 0) {
                        int button = oldValue + 100;
                        String name;
                        if (button == 0) {
                           name = "key.mouse.left";
                        } else if (button == 1) {
                           ...
                           name = "key.mouse." + (button + 1);
                        }

                        return Pair.of((Dynamic)entry.getKey(), ((Dynamic)entry.getValue()).createString(name));
                     } else {
                        String name = (String)MAP.getOrDefault(oldValue, "key.unknown");
                        return Pair.of((Dynamic)entry.getKey(), ((Dynamic)entry.getValue()).createString(name));
                     }
                  } else {
