...
public class ChunkStructuresTemplateRenameFix extends DataFix {
   ...
   private Dynamic<?> fixChildren(final Dynamic<?> structure) {
      return structure.update("Children", (children) -> structure.createList(children.asStream().map((child) -> this.fixTag(structure, child))));
   }

   private Dynamic<?> fixTag(final Dynamic<?> structure, Dynamic<?> child) {
      String id = structure.get("id").asString("");
      if (RENAMES.containsKey(id)) {
         Pair<String, ImmutableMap<String, String>> data = (Pair)RENAMES.get(id);
         if (((String)data.getFirst()).equals(child.get("id").asString(""))) {
            String template = child.get("Template").asString("");
            child = child.set("Template", child.createString((String)((ImmutableMap)data.getSecond()).getOrDefault(template, template)));
         }
      }
