...
public class V1451_6 extends NamespacedSchema {
   ...
   protected static final HookFunction UNPACK_OBJECTIVE_ID = new HookFunction() {
      public <T> T apply(DynamicOps<T> var1, T var2) {
         ...
         return ((Dynamic)DataFixUtils.orElse(var3.get("CriteriaName").asString().get().left().map((var0) -> {
            int var1 = var0.indexOf(58);
            if (var1 < 0) {
               return Pair.of("_special", var0);
            } else {
               try {
                  ResourceLocation var2 = ResourceLocation.method_104(var0.substring(0, var1), '.');
                  ResourceLocation var3 = ResourceLocation.method_104(var0.substring(var1 + 1), '.');
   ...
   protected static final HookFunction REPACK_OBJECTIVE_ID = new HookFunction() {
      ...
      public <T> T apply(DynamicOps<T> var1, T var2) {
         Dynamic var3 = new Dynamic(var1, var2);
         Optional var4 = var3.get("CriteriaType").get().get().left().flatMap((var2x) -> {
            Optional var3x = var2x.get("type").asString().get().left();
            Optional var4 = var2x.get("id").asString().get().left();
            if (var3x.isPresent() && var4.isPresent()) {
               String var5 = (String)var3x.get();
               if (var5.equals("_special")) {
                  return Optional.of(var3.createString((String)var4.get()));
               } else {
                  String var10001 = this.packWithDot(var5);
                  return Optional.of(var2x.createString(var10001 + ":" + this.packWithDot((String)var4.get())));
               }
            } else {
