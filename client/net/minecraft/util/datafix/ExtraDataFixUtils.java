...
public class ExtraDataFixUtils {
      ...
      if (!var1.isEmpty()) {
         var3 = var3.set(
            ...
            var2.createMap(
               var1.entrySet()
                  .stream()
                  .collect(Collectors.toMap(var1x -> var2.createString((String)var1x.getKey()), var1x -> var2.createString((String)var1x.getValue())))
            )
         );
      }
   ...
   public static Dynamic<?> fixStringField(Dynamic<?> var0, String var1, UnaryOperator<String> var2) {
      return var0.update(var1, var2x -> (Dynamic)DataFixUtils.orElse(var2x.asString().map(var2).map(var0::createString).result(), var2x));
   }
}
