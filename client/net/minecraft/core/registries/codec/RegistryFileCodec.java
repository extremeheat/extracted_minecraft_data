...
public final class RegistryFileCodec<E> implements Codec<Holder<E>> {
   ...
   public static <E> Codec<Holder<E>> create(final ResourceKey<? extends Registry<E>> registryKey, final Codec<E> elementCodec, final boolean allowInline) {
      return new RegistryFileCodec<Holder<E>>(registryKey, elementCodec, allowInline);
   }
      ...
      if (ops instanceof RegistryOps<?> registryOps) {
         ...
         if (decoded.result().isEmpty()) {
            ...
            if (maybeLookup.isEmpty()) {
               ...
               return ((DataResult)lookup.get(elementKey).map(DataResult::success).orElseGet(() -> DataResult.error(() -> {
