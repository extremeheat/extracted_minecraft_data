...
public record TestData<EnvironmentType>(EnvironmentType environment, ResourceKey<Level> dimension, Identifier structure, int maxTicks, int setupTicks, boolean required, Rotation rotation, boolean manualOnly, int maxAttempts, int requiredSuccesses, boolean skyAccess, int padding) {
   ...
   public TestData(final EnvironmentType environment, final Identifier structure, final int maxTicks, final int setupTicks, final boolean required, final Rotation rotation) {
      this(environment, Level.OVERWORLD, structure, maxTicks, setupTicks, required, rotation, false, 1, 1, false, 0);
   }
   ...
   public <T> TestData<T> map(final Function<EnvironmentType, T> mapper) {
      return new TestData<T>(mapper.apply(this.environment), this.dimension, this.structure, this.maxTicks, this.setupTicks, this.required, this.rotation, this.manualOnly, this.maxAttempts, this.requiredSuccesses, this.skyAccess, this.padding);
   }
}
