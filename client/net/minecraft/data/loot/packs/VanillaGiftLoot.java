...
public record VanillaGiftLoot(HolderLookup.Provider registries) implements LootTableSubProvider {
   ...
   public void generate(final BiConsumer<ResourceKey<LootTable>, LootTable.Builder> output) {
      ...
      output.accept(BuiltInLootTables.SHEPHERD_GIFT, LootTable.lootTable().withPool(LootPool.lootPool().setRolls(ConstantValue.exactly(1.0F)).addAll(Items.WOOL.asList().stream().map(LootItem::lootTableItem).toList())));
