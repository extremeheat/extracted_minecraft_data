...
public record VanillaBlockInteractLoot(HolderLookup.Provider registries) implements LootTableSubProvider {
   ...
   public void generate(final BiConsumer<ResourceKey<LootTable>, LootTable.Builder> output) {
      ...
      output.accept(BuiltInLootTables.TILL_ROOTED_DIRT, LootTable.lootTable().withPool(LootPool.lootPool().setRolls(ConstantValue.exactly(1.0F)).add(LootItem.lootTableItem(Items.HANGING_ROOTS).apply(SetItemCountFunction.setCount(ConstantValue.exactly(1.0F))))));
   }
}
