...
public class GpuBuffer implements AutoCloseable {
   ...
   @Nullable
   public GpuBuffer.ReadView read(int var1, int var2) {
      if (this.closed) {
         throw new IllegalStateException("Buffer already closed");
         ...
         throw new IllegalArgumentException(
            "Cannot read more data than this buffer can hold (attempting to read " + var2 + " bytes at offset " + var1 + " from " + this.size + " size buffer)"
         );
      } else {
         this.bind();
         ByteBuffer var3 = GlStateManager._glMapBufferRange(this.type.id, var1, var2, 1);
         return var3 == null ? null : new GpuBuffer.ReadView(this.type.id, var3);
      }
   }
