...
public record IntermediaryShaderModule(String name, ByteBuffer spirv, List<SpvUniformBuffer> uniformBuffers, List<SpvSampler> samplers, List<SpvVariable> outputs, List<SpvVariable> inputs) {
   ...
   public static IntermediaryShaderModule createFromSpirv(final String filename, final ByteBuffer spirv) throws ShaderCompileException {
      ...
      try {
         ...
         try {
            throwIfError(Spvc.spvc_context_parse_spirv(context, spirv.asIntBuffer(), (long)(spirv.remaining() / 4), pointer), "Couldn't parse spirv");
