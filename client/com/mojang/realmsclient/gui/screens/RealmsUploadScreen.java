...
public class RealmsUploadScreen extends RealmsScreen implements RealmsWorldUploadStatusTracker {
   ...
   private void onCancel() {
      this.cancelled = true;
      RealmsWorldUpload var1 = this.currentUpload.get();
      if (var1 != null) {
         var1.cancel();
      } else {
   ...
   private void upload() {
      ...
      RealmsWorldOptions var2 = RealmsWorldOptions.createFromSettings(
      ...
      if (!this.currentUpload.compareAndSet(null, var3)) {
         ...
         var3.packAndUpload().handleAsync((var1x, var2x) -> {
            if (var2x != null) {
               if (var2x instanceof CompletionException var3x) {
                  var2x = var3x.getCause();
               }

               if (var2x instanceof RealmsUploadException var4) {
