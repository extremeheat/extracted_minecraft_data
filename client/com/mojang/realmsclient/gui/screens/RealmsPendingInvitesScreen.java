...
public class RealmsPendingInvitesScreen extends RealmsScreen {
   ...
   public void init() {
      ...
      this.acceptButton = (Button)this.addRenderableWidget(new Button(this.width / 2 - 174, this.height - 32, 100, 20, Component.translatable("mco.invites.button.accept"), (var1) -> {
      ...
      this.rejectButton = (Button)this.addRenderableWidget(new Button(this.width / 2 + 74, this.height - 32, 100, 20, Component.translatable("mco.invites.button.reject"), (var1) -> {
         ...
         (new Thread("Realms-reject-invitation") {
            public void run() {
               try {
                  ...
                  var1x.rejectInvitation(((Entry)RealmsPendingInvitesScreen.this.pendingInvitationSelectionList.children().get(var1)).pendingInvite.invitationId);
                  RealmsPendingInvitesScreen.this.minecraft.execute(() -> {
      ...
      if (var1 < this.pendingInvitationSelectionList.getItemCount()) {
         (new Thread("Realms-accept-invitation") {
            public void run() {
               try {
                  RealmsClient var1x = RealmsClient.create();
                  var1x.acceptInvitation(((Entry)RealmsPendingInvitesScreen.this.pendingInvitationSelectionList.children().get(var1)).pendingInvite.invitationId);
                  RealmsPendingInvitesScreen.this.minecraft.execute(() -> {
                     RealmsPendingInvitesScreen.this.updateList(var1);
                  });
               } catch (RealmsServiceException var2) {
   ...
   class PendingInvitationSelectionList extends RealmsObjectSelectionList<Entry> {
   ...
   private class Entry extends ObjectSelectionList.Entry<Entry> {
      ...
      public Component getNarration() {
         Component var1 = CommonComponents.joinLines(Component.literal(this.pendingInvite.worldName), Component.literal(this.pendingInvite.worldOwnerName), Component.literal(RealmsUtil.convertToAgePresentationFromInstant(this.pendingInvite.date)));
