...
public class RealmsPlayerScreen extends RealmsScreen {
   ...
   public void init() {
      ...
      this.addRenderableWidget(new Button(this.column2X, row(1), this.columnWidth + 10, 20, Component.translatable("mco.configure.world.buttons.invite"), (var1x) -> {
         this.minecraft.setScreen(new RealmsInviteScreen(this.lastScreen, this, this.serverData));
      }));
      this.removeButton = (Button)this.addRenderableWidget(new Button(this.column2X, row(7), this.columnWidth + 10, 20, Component.translatable("mco.configure.world.invites.remove.tooltip"), (var1x) -> {
         this.uninvite(this.player);
      }));
      this.opdeopButton = (Button)this.addRenderableWidget(new Button(this.column2X, row(9), this.columnWidth + 10, 20, Component.translatable("mco.configure.world.invites.ops.tooltip"), (var1x) -> {
   ...
   public void render(PoseStack var1, int var2, int var3, float var4) {
      ...
      var7.vertex(0.0, (double)this.height, 0.0).uv(0.0F, (float)(this.height - var5) / 32.0F + 0.0F).color(64, 64, 64, 255).endVertex();
      var7.vertex((double)this.width, (double)this.height, 0.0).uv((float)this.width / 32.0F, (float)(this.height - var5) / 32.0F + 0.0F).color(64, 64, 64, 255).endVertex();
      var7.vertex((double)this.width, (double)var5, 0.0).uv((float)this.width / 32.0F, 0.0F).color(64, 64, 64, 255).endVertex();
      var7.vertex(0.0, (double)var5, 0.0).uv(0.0F, 0.0F).color(64, 64, 64, 255).endVertex();
      var6.end();
      if (this.serverData != null && this.serverData.players != null) {
         this.font.draw(var1, (Component)Component.empty().append(INVITED_LABEL).append(" (").append(Integer.toString(this.serverData.players.size())).append(")"), (float)this.column1X, (float)row(0), 10526880);
      } else {
   ...
   static enum UserAction {
      ...
      // $FF: synthetic method
      private static UserAction[] $values() {
