...
public class InventoryChangeTrigger extends SimpleCriterionTrigger<InventoryChangeTrigger.TriggerInstance> {
   ...
   public static record TriggerInstance(Optional<ContextAwarePredicate> player, InventoryChangeTrigger.TriggerInstance.Slots slots, List<ItemPredicate> items)
      ...
      public TriggerInstance(Optional<ContextAwarePredicate> player, InventoryChangeTrigger.TriggerInstance.Slots slots, List<ItemPredicate> items) {
         super();
         this.player = player;
         this.slots = slots;
         this.items = items;
      ...
      public static Criterion<InventoryChangeTrigger.TriggerInstance> hasItems(ItemPredicate.Builder... var0) {
         return hasItems(Stream.of(var0).map(ItemPredicate.Builder::build).toArray(ItemPredicate[]::new));
      }
      ...
      public static record Slots(MinMaxBounds.Ints occupied, MinMaxBounds.Ints full, MinMaxBounds.Ints empty) {
         ...
         public boolean matches(int var1, int var2, int var3) {
            if (!this.full.matches(var1)) {
               ...
               return !this.empty.matches(var2) ? false : this.occupied.matches(var3);
            }
         }
      }
   }
