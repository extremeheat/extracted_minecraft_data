...
public class FilteredFunction extends LootItemConditionalFunction {
   public static final MapCodec<FilteredFunction> CODEC = RecordCodecBuilder.mapCodec(
      var0 -> commonFields(var0)
            .and(
               var0.group(
                  ItemPredicate.CODEC.fieldOf("item_filter").forGetter(var0x -> var0x.filter),
                  LootItemFunctions.ROOT_CODEC.fieldOf("modifier").forGetter(var0x -> var0x.modifier)
               )
            )
   ...
   public ItemStack run(ItemStack var1, LootContext var2) {
      return this.filter.test(var1) ? this.modifier.apply(var1, var2) : var1;
   }
