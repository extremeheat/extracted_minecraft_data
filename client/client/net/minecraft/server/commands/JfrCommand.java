...
public class JfrCommand {
   ...
   private static final DynamicCommandExceptionType DUMP_FAILED = new DynamicCommandExceptionType(
      var0 -> Component.translatableEscape("commands.jfr.dump.failed", var0)
   );
   ...
   public static void register(CommandDispatcher<CommandSourceStack> var0) {
      var0.register(
         (LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)Commands.literal("jfr").requires(var0x -> var0x.hasPermission(4)))
               .then(Commands.literal("start").executes(var0x -> startJfr((CommandSourceStack)var0x.getSource()))))
            .then(Commands.literal("stop").executes(var0x -> stopJfr((CommandSourceStack)var0x.getSource())))
      );
   }
   ...
   private static int stopJfr(CommandSourceStack var0) throws CommandSyntaxException {
      try {
         ...
         MutableComponent var3 = Component.literal(var1.toString())
            .withStyle(ChatFormatting.UNDERLINE)
            .withStyle(
               var1x -> var1x.withClickEvent(new ClickEvent(ClickEvent.Action.COPY_TO_CLIPBOARD, var2.toString()))
